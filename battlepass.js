...
export async function claimBpTaskReward(uid, taskId, gameState, CASES = {}) {
  ...
  const c = CASES[def.bonus.caseId];
  const caseItem = {
    id: "bp_" + Math.random().toString(36).slice(2),
    type: "case", caseId: def.bonus.caseId,
    name: c ? c.name : (def.bonus.label || def.bonus.caseId),  // ← виправлено
    img: c ? c.img : "img/cases/" + def.bonus.caseId + ".png",
    source: "battlepass",
