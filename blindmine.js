...
function renderIntro() {
  ...
  _root.innerHTML = `
    <div class="bm-wrap">
      <div class="bm-intro">Той самий сапер, але без підказок — жодних цифр про сусідні міни. Клікай клітинки, поки не підірвешся. Кожна відкрита безпечна клітинка коштуватиме ${BLIND_MINE_REWARD_PER_CELL} нікуси, нарахування — одразу після підриву або повної зачистки поля.</div>
...
async function endRound(cleared) {
  ...
  try {
    ...
    _deps.updateBalanceDisplay?.();
    _deps.showToast?.(`+${res.reward} нікусів за ${openedCount} відкритих клітинок`, cleared ? "success" : "");
