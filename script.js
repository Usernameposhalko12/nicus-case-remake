...
// ── СПІН МЕЛЛКОІНС З АНІМАЦІЄЮ ──────────────────────────

function buildMellSpinStrip(winIdx, prizeEntry) {
  const COUNT = 60;
  const strip = [];
  ...
  for (let i = 0; i < COUNT; i++) {
    if (i === winIdx) {
      strip.push({ ...prizeEntry, _isWin: true });
      continue;
    }
    // Зважений рандом для решти
    let r = Math.random() * totalW;
    let picked = MELL_SPIN_TABLE[MELL_SPIN_TABLE.length - 1];
...
function mellSpinCardHtml(entry, ITEM_W) {
  // Визначаємо колір і зображення
  let color = "#5ddb8a";
  ...
  if (entry.type === "item") {
    ...
    color = r ? r.color : "#888";
    rarName = r ? r.name : "";
    // Шукаємо картинку з CASES
    let found = null;
    Object.values(CASES).forEach(c => {
      const f = c.items.find(i => i.id === entry.itemId);
      if (f) found = f;
    });
    imgSrc = found ? found.img : "img/placeholder.png";
  } else if (entry.type === "case") {
    const c = CASES[entry.caseId];
    imgSrc = c ? c.img : "img/cases/mellstroy.png";
    color = "#ffe066";
    rarName = "Кейс";
  }

  const glow = entry.rarity === "special" || entry.rarity === "secret"
  ...
  return `<div class="roulette-card" style="border-color:${color};min-width:${ITEM_W}px;position:relative;${glow}">
    ...
    <div style="font-size:10px;color:${color};margin-top:4px;font-weight:700;text-align:center;">${rarName||entry.label||""}</div>
  </div>`;
}

// ── КОЛЕСО ФОРТУНИ (CANVAS) ──────────────────────────────

function buildMellWheelSegments() {
  // Зважені сегменти — чим більша вага, тим більший кут
  const totalW = MELL_SPIN_TABLE.reduce((s, x) => s + x.weight, 0);
  return MELL_SPIN_TABLE.map((entry, i) => {
    ...
    if (entry.type === "item") {
      const r = RARITIES[entry.rarity];
      color = r ? r.color + "cc" : "#555";
    } else {
      color = "#ffe06699";
    }
    const altColor = i % 2 === 0 ? color : color.replace("cc","88").replace("99","55");
    return { ...entry, angle, color: altColor };
  });
}
...
function drawWheel(canvas, segments, rotation) {
  ...
  // Блискучі кружечки по ободу
  const dotCount = 16;
  for (let i = 0; i < dotCount; i++) {
    const a = (i / dotCount) * Math.PI * 2 + rotation * 0.5;
...
function drawArrow(arrowCanvas) {
  ...
  // Трикутна стрілка вказує вниз (на колесо зверху)
  const ax = w / 2, ay = h - 2;
  ctx.beginPath();
  ctx.moveTo(ax, ay);
  ctx.lineTo(ax - 14, 0);
...
async function openMellSpinModal() {
  ...
  openModal(`
    ...
    <div style="text-align:center;margin-bottom:10px;">
      <div style="font-size:22px;font-weight:800;color:#ffe066;">${coins} 🪙</div>
      <div style="color:var(--text-muted);font-size:13px;">Твої МеллКоінс</div>
    </div>

    <!-- Колесо -->
    <div style="position:relative;display:flex;flex-direction:column;align-items:center;margin-bottom:14px;">
      <canvas id="mell-arrow-canvas" width="32" height="36"
        style="display:block;margin-bottom:-2px;z-index:2;position:relative;"></canvas>
      <canvas id="mell-wheel-canvas" width="300" height="300"
        style="display:block;max-width:100%;border-radius:50%;
               box-shadow:0 0 32px rgba(255,215,0,0.25),0 0 8px rgba(0,0,0,0.5);"></canvas>
    </div>
    ...
    <details style="margin-top:12px;">
      <summary style="font-size:12px;font-weight:700;color:var(--text-muted);cursor:pointer;">📊 Шанси</summary>
  ...
  // Зберігаємо сегменти глобально для спіну
...
async function doMellSpinAction() {
  ...
  // Визначаємо приз через Firestore
  let prize;
  try {
  ...
  // Знаходимо який сегмент відповідає призу
  // Шукаємо по label (бо doMellSpin повертає prize з таблиці)
  ...
  // Обчислюємо кут зупинки колеса
  // Стрілка вказує на TOP (кут = -π/2 від центру)
  // Знаходимо середній кут цільового сегменту
  let segStart = 0;
  for (let i = 0; i < targetIdx; i++) segStart += segments[i].angle;
  const segMid    = segStart + segments[targetIdx].angle / 2;
  // Хочемо щоб segMid опинився під стрілкою (кут -π/2)
  // Стрілка = top = -Math.PI/2
  // finalRotation: -Math.PI/2 - segMid + випадковий розкид всередині сегменту
  const scatter   = (Math.random() - 0.5) * segments[targetIdx].angle * 0.6;
  const targetAngle = -Math.PI / 2 - segMid - scatter;
  // Додаємо 5-8 повних обертів
  ...
  function animate(now) {
    ...
    const current = startAngle + easeOut(t) * finalAngle;

    if (wheelCanvas) drawWheel(wheelCanvas, segments, current);

    if (t < 1) {
      requestAnimationFrame(animate);
    } else {
      // Зупинились
      showSpinResult(prize, resultEl, btnsEl, againBtn);
    }
  }
...
function showSpinResult(prize, resultEl, btnsEl, againBtn) {
  let color = "#5ddb8a";
  if (prize.type === "item") {
  ...
  if (resultEl) {
    ...
    resultEl.innerHTML = `
      <div style="border:2px solid ${color};border-radius:14px;padding:16px;
        ...
        <div style="font-size:16px;font-weight:800;color:${color};">${prize.label}</div>
  ...
  // Оновлюємо відображення МеллКоінс на сторінці крафту
...
function injectMellCraftStyles() {
  ...
  s.textContent = `
    .mellcraft-header {
      ...
      border: 1.5px solid var(--border); border-radius: 14px; margin-bottom: 14px;
    }
    .mellcraft-stat {
      display: flex; align-items: center; gap: 8px; flex: 1;
      background: var(--bg-card2); border-radius: 10px; padding: 10px 12px;
    }
    .mellcraft-stat-icon { font-size: 22px; }
    .mellcraft-stat-val  { font-size: 20px; font-weight: 800; color: #ffe066; }
    .mellcraft-stat-label{ font-size: 11px; color: var(--text-muted); font-weight: 600; }
    .mell-spin-btn {
    ...
    .mell-spin-btn:hover { filter: brightness(1.1); transform: translateY(-2px); }
    ...
    .mellhp-bar {
      ...
      border-radius: 9px; overflow: visible; margin: 4px 0 18px;
    }
    .mellhp-fill {
      height: 100%; border-radius: 9px;
      background: linear-gradient(90deg, #5ddb8a, #00c07f);
      transition: width .5s;
    }
    .mellhp-milestone {
      position: absolute; top: 50%; transform: translate(-50%, -50%);
      font-size: 16px; cursor: default; filter: drop-shadow(0 1px 3px rgba(0,0,0,.4));
    }
    .mellhp-milestone.done { opacity: .5; }
...
window.showSpinResult         = showSpinResult;

window.skipMellSpin = skipMellSpin;
