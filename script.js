...
async function _startMellWheelSpin(power) {
  ...
  let prize;
  try {
    // Передаємо username напряму з currentUser
  ...
  // Оновлюємо стан з Firestore одразу після спіну
  try {
    const snap = await getDoc(doc(db, "users", currentUser.uid));
    if (snap.exists()) {
  ...
  // Визначаємо цільовий кут для призу
...
async function doMellCraftAction() {
  ...
  try {
    ...
    // Оновлюємо з Firestore і одразу апдейтимо UI
    const snap = await getDoc(doc(db, "users", currentUser.uid));
    if (snap.exists()) {
      gameState.mellCoins     = snap.data().mellCoins     || 0;
      gameState.mellHpContrib = snap.data().mellHpContrib || 0;
