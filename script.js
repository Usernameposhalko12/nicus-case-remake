...
async function _startMellWheelSpin(power) {
  ...
  // Оптимістично знімаємо монети одразу
  ...
  // Firestore у фоні — не блокує анімацію
  ...
  // Локально визначаємо приз для анімації
  const totalW = MELL_SPIN_TABLE.reduce((s, x) => s + x.weight, 0);
  let rnd = Math.random() * totalW;
  let localPrize = MELL_SPIN_TABLE[MELL_SPIN_TABLE.length - 1];
  for (const entry of MELL_SPIN_TABLE) {
    rnd -= entry.weight;
    if (rnd <= 0) { localPrize = entry; break; }
  }

  const targetIdx = MELL_SPIN_TABLE.indexOf(localPrize);
  const localTargetIdx = targetIdx >= 0 ? targetIdx : 0;

  let segStart = 0;
  for (let i = 0; i < localTargetIdx; i++) segStart += segments[i].angle;
  const segMid = segStart + segments[localTargetIdx].angle / 2;
  ...
  const totalRot = 6 * Math.PI * 2 + diff;
