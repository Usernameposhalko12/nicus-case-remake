...
async function _startMellWheelSpin(power) {
  // Синхронізуємо актуальний баланс перед перевіркою
  try {
    const snap = await getDoc(doc(db, "users", currentUser.uid));
    if (snap.exists()) {
  ...
  } catch(e) { console.warn("sync mellCoins error:", e); }
  ...
  gameState.mellCoins = (gameState.mellCoins || 0) - MELL_SPIN_COST;
  const coinsEl = document.getElementById("mellcoins-display");
  if (coinsEl) coinsEl.textContent = gameState.mellCoins;
  ...
  // Спочатку крутимо 5+ обертів без зупинки на конкретному сегменті
  const spinRounds = 6 * Math.PI * 2;
  const freeSpinDuration = 2500 + power * 2000;
  ...
  // Фаза 1: вільне кручення (поки Firestore думає)
  await new Promise(resolve => {
    function animateFree(now) {
      const c = document.getElementById("mell-wheel-canvas");
      if (!c) { resolve(); return; }
      const elapsed = now - startTime;
      const t = Math.min(elapsed / freeSpinDuration, 1);
      _mellSpring.rotation = startRot + easeInOut(t) * spinRounds;
      drawWheel(c, segments, _mellSpring.rotation);
      if (t < 1) {
        _mellSpring.animId = requestAnimationFrame(animateFree);
      } else {
        resolve();
      }
    }
  ...
  if (!prize) {
    ...
    showToast("❌ Помилка сервера, монети повернуто", "error");
    return;
  }

  // Фаза 2: доповертаємо до правильного сегменту
  ...
  const curNorm = ((_mellSpring.rotation % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2);
  ...
  const snapStartRot = _mellSpring.rotation;
  ...
  await new Promise(resolve => {
    function animateSnap(now) {
      ...
      _mellSpring.rotation = snapStartRot + easeOut(t) * diff;
      drawWheel(c, segments, _mellSpring.rotation);
      if (t < 1) {
        _mellSpring.animId = requestAnimationFrame(animateSnap);
      } else {
        resolve();
      }
    }
