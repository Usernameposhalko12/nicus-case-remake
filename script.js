...
// ЗАМІНИТИ buildMellWheelSegments:
function buildMellWheelSegments() {
  const count = MELL_SPIN_TABLE.length; // 7 сегментів
  const equalAngle = (Math.PI * 2) / count; // однаковий кут для кожного
...
async function renderMellCraft() {
  ...
  page.innerHTML = `
    <!-- Хедер: МеллКоінс / особистий внесок -->
  ...
  // Рендеримо крафт-слоти
  _mellCraftSelected = [];
  mellRenderSlots();
  mellRenderPicker();

  // Підписка на глобальний МеллХП
