...
// ─── 1. Малюємо золоту медаль через canvas (як CS2) ────────────
// Повертає data-URL PNG розміром size×size
function drawSpecialMedalCanvas(size = 120) {
  ...
  // ── Лавровий вінок (ліво) ──
  drawLaurel(ctx, cx, cy, r, -1, size);
  // ── Лавровий вінок (право) ──
  drawLaurel(ctx, cx, cy, r, 1, size);

  // ── Зовнішнє кільце (темне золото) ──
  ctx.beginPath();
  ctx.arc(cx, cy, r, 0, Math.PI * 2);
  const ringOuter = ctx.createRadialGradient(cx - r * 0.2, cy - r * 0.2, r * 0.1, cx, cy, r);
  ringOuter.addColorStop(0,   "#ffe066");
  ringOuter.addColorStop(0.4, "#f5b800");
  ringOuter.addColorStop(0.7, "#c8860a");
  ringOuter.addColorStop(1,   "#7a4e00");
  ctx.fillStyle = ringOuter;
  ctx.fill();

  // ── Внутрішнє поле медалі ──
  ctx.beginPath();
  ctx.arc(cx, cy, r * 0.78, 0, Math.PI * 2);
  const inner = ctx.createRadialGradient(cx - r * 0.25, cy - r * 0.3, r * 0.05, cx, cy, r * 0.78);
  inner.addColorStop(0,   "#fff3a0");
  inner.addColorStop(0.35,"#f5c800");
  inner.addColorStop(0.7, "#d4900e");
  inner.addColorStop(1,   "#a06008");
  ctx.fillStyle = inner;
  ctx.fill();
  ...
  // Тінь
  ctx.shadowColor = "rgba(80,30,0,0.7)";
  ...
  tg.addColorStop(0.4, "#ffe566");
  tg.addColorStop(1,   "#b87000");
  ctx.fillStyle = tg;
  ctx.fillText("?", cx, cy + size * 0.025);
  ...
  // ── Відблиск ──
  ctx.beginPath();
  ctx.ellipse(cx - r * 0.22, cy - r * 0.28, r * 0.28, r * 0.14, -Math.PI / 4, 0, Math.PI * 2);
...
function drawLaurel(ctx, cx, cy, r, side, size) {
  // side: -1 = ліво, 1 = право
  const leafCount = 7;
  const startAng  = side === -1 ? Math.PI * 0.55 : Math.PI * 0.45;
  const endAng    = side === -1 ? Math.PI * 1.45 : -Math.PI * 0.45;
  const step      = (endAng - startAng) / (leafCount - 1);

  ctx.save();
  for (let i = 0; i < leafCount; i++) {
    const ang = startAng + step * i;
    const lx  = cx + Math.cos(ang) * r * 1.08;
    const ly  = cy + Math.sin(ang) * r * 1.08;
    const lw  = size * 0.055;
    const lh  = size * 0.12;
    ...
    const lg = ctx.createRadialGradient(0, -lh * 0.3, 0, 0, 0, lh);
    lg.addColorStop(0,   "#c8e850");
    lg.addColorStop(0.5, "#7ab020");
    lg.addColorStop(1,   "#3a6008");
    ctx.fillStyle = lg;
    ctx.fill();
  ...
  ctx.beginPath();
  if (side === -1) {
    ctx.arc(cx, cy, r * 1.1, Math.PI * 0.5, Math.PI * 1.5);
  } else {
    ctx.arc(cx, cy, r * 1.1, -Math.PI * 0.5, Math.PI * 0.5);
  }
...
function rouletteCardHtml(item, ITEM_W) {
  ...
  // Спеціалка (і вінпос-спеціалка, і декорація) — малюємо медаль
  ...
  const glow     = isSpecial
    ? `box-shadow:0 0 18px 4px rgba(255,200,0,0.55),0 0 6px 1px rgba(255,165,0,0.4);`
    : "";
  ...
  return `<div class="roulette-card" style="border-color:${color};min-width:${ITEM_W}px;position:relative;${glow}">
    ${decorBadge}
    <img src="${imgSrc}" onerror="this.src='img/placeholder.png'" style="${isSpecial ? "filter:drop-shadow(0 2px 8px rgba(255,180,0,0.6));" : ""}">
...
function finishCaseOpen() {
  ...
  // Для спеціалки — показуємо медаль у результаті
  const resultImgSrc = isSpecialWin
    ? getSpecialMedalUrl()
    : (newItem.img || "img/placeholder.png");
    ...
    resultEl.innerHTML = `
      ...
      ">
        ...
        <img src="${resultImgSrc}"
          ...
          style="width:${isSpecialWin ? 90 : 80}px;height:${isSpecialWin ? 90 : 80}px;object-fit:contain;${isSpecialWin ? "filter:drop-shadow(0 4px 16px rgba(255,190,0,0.7));" : ""}">
        ...
        ${isSpecialWin ? '<div style="color:#ffe066;font-size:12px;margin-top:6px;letter-spacing:1px;">✨ НАДРІДКІСНИЙ ПРЕДМЕТ ✨</div>' : ''}
      </div>`;
  }
...
async function renderClan() {
  ...
  // ← ДЕБАГ БЛОК
  showToast("Clan ID: " + gameState.clan, "");
  ...
  try {
    ...
    // ← ДЕБАГ
    if (!clan) {
      showToast("getClan повернув null!", "error");
...
if (_activeClanTab !== "members") {
  setTimeout(() => switchClanTab(_activeClanTab), 0);
    ...
    showToast("Клан знайдено: " + clan.name, "success");
  ...
  onAuthStateChanged(auth, async (firebaseUser) => {
    if (firebaseUser) {
      ...
      _unsubProfile = onSnapshot(doc(db, "users", firebaseUser.uid), (snap) => {
        ...
        updateBalanceDisplay();
        renderMain();
        // Авто-оновлення сторінок що залежать від профілю
