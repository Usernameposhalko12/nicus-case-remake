...
async function _startMellWheelSpin(power) {
  ...
  // 1. Спочатку отримуємо приз із сервера
  let prize;
  try {
    prize = await doMellSpin(currentUser.uid, gameState);
  } catch(e) {
  ...
  // 2. Визначаємо цільовий кут для призу
  ...
  // Стрілка зверху = -π/2, тому ціль: rotation = -π/2 - segMid
  ...
  // Мінімум (3 + power * 6) повних обертів + залишок до призу
  const minSpins = 3 + power * 6;
  const totalRot = minSpins * Math.PI * 2 + diff;

  // 3. Анімація одразу до призу
  const duration  = 3500 + power * 3500;
...
function _snapWheelToPrize(prize, segments, canvas, onDone) {
  ...
  // Стрілка зверху = -π/2
  // Хочемо: rotation ≡ -π/2 - segMid (mod 2π)
  const targetAngle = -Math.PI / 2 - segMid;

  // Нормалізуємо поточне в [0, 2π)
  const curNorm = (((_mellSpring.rotation % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2));
  const tgtNorm = (((targetAngle % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2));

  // Завжди крутимо ВПЕРЕД (за годинниковою) — diff завжди >= 0
  let diff = tgtNorm - curNorm;
  if (diff < 0) diff += Math.PI * 2;
  // Мінімум чверть оберту щоб було видно рух
  if (diff < Math.PI * 0.3) diff += Math.PI * 2;
