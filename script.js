...
function updateBalanceDisplay() {
  const bal  = gameState.balance ?? 0;
   _trackBpBalance(bal); // авто-відстеження
  ...
  if (navLogo && window.innerWidth > 900) {
    ...
    // Додай одразу після let _isSaving = false;
let _bpPrevBalance = null;

async function _trackBpBalance(newBal) {
  if (_bpPrevBalance === null) { _bpPrevBalance = newBal; return; }
  const diff = newBal - _bpPrevBalance;
  if (diff > 0) await bpHookEarn(diff);
  else if (diff < 0) await bpHookSpend(-diff);
  _bpPrevBalance = newBal;
}
...
const earnedCoins = (trade?.offerItems||[])
  .filter(i => i._type === "balance")
  .reduce((s, i) => s + (i._amount||0), 0);
if (earnedCoins > 0) await bpHookEarn(earnedCoins);
