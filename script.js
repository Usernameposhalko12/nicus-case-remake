...
// КЛЮЧОВИЙ ФІЛ: поки true — onSnapshot не перезаписує gameState (захист від race condition)
let _isSaving     = false;
...
function openCase(invIndex) {
  ...
  const ITEM_W = 120, ITEM_GAP = 10, TOTAL_W = 130, COUNT = 60;
  const strip  = [];
...
async function buyFromMarketItem(docId) {
  ...
  try {
    ...
    if (!gameState.inventory) gameState.inventory = [];
    gameState.inventory.push({ ...l.item, id: generateId(), obtainedAt: Date.now() });
    await saveData();
    updateBalanceDisplay();
    await renderMarket();
...
async function renderTrades() {
  ...
  try {
    ...
    list.innerHTML = trades.map(trade => {
      ...
      const offerHtml = (trade.offerItems||[]).map(i => '<div class="trade-item"><img src="' + (i.img||'img/placeholder.png') + '" onerror="this.src=\'img/placeholder.png\'"><span>' + i.name + '</span></div>').join("");
      const wantHtml  = (trade.wantItems||[]).map(i => '<div class="trade-item"><img src="' + (i.img||'img/placeholder.png') + '" onerror="this.src=\'img/placeholder.png\'"><span>' + i.name + '</span></div>').join("");
      return '<div class="trade-card">' +
        '<div class="trade-header"><span>' + (isSender ? "📤 → " + trade.toUsername : "📥 від " + trade.fromUsername) + '</span>' +
        '<span class="trade-status">' + (statusMap[trade.status] || trade.status) + '</span></div>' +
        '<div class="trade-items"><div><div style="color:var(--text-muted);font-size:12px;margin-bottom:6px;">Ти даєш:</div>' + offerHtml + '</div>' +
        '<div style="font-size:24px;align-self:center;">⇄</div>' +
        '<div><div style="color:var(--text-muted);font-size:12px;margin-bottom:6px;">Ти отримуєш:</div>' + wantHtml + '</div></div>' +
        (!isSender && trade.status === "pending"
          ? '<div style="display:flex;gap:8px;margin-top:12px;"><button class="btn-primary" onclick="acceptTrade(\'' + trade.docId + '\')">✅ Прийняти</button><button class="btn-secondary" onclick="declineTrade(\'' + trade.docId + '\')">❌ Відхилити</button></div>' : '') +
        (isSender && trade.status === "pending"
    ...
    list.innerHTML = '<div class="empty-state">❌ Помилка: ' + e.message + '<br><small>Можливо потрібен Firebase composite index для колекції trades. Деталі в консолі.</small></div>';
  }
}
...
async function showTradeModal(invIdx) {
  ...
  openModal(
    ...
    '<div class="trade-item" style="background:var(--bg-card);padding:10px;border-radius:8px;"><img src="' + (item.img||'img/placeholder.png') + '" onerror="this.src=\'img/placeholder.png\'"><span>' + item.name + '</span></div>' +
    '<div style="font-weight:600;margin:16px 0 8px;">Що хочеш отримати:</div>' +
    '<input type="text" id="trade-want" placeholder="Назва предмета..." style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);background:var(--bg-card);color:var(--text);font-size:14px;margin-bottom:16px;box-sizing:border-box;">' +
    '<div style="display:flex;gap:8px;"><button class="btn-primary" style="flex:1;" onclick="sendTrade(' + invIdx + ')">📤 Надіслати</button><button class="btn-secondary" style="flex:1;" onclick="closeModal()">Скасувати</button></div>'
  );
}
...
async function sendTrade(invIdx) {
  ...
  if (!item)   { showToast("Предмет не знайдено!", "error"); return; }
  try {
    await sendTradeRequest(currentUser.uid, currentUser.username, target, [{...item}], [{name:want,img:"img/items/placeholder.png"}]);
    closeModal();
    showToast("📤 Трейд надіслано!", "success");
  } catch (e) { showToast(e.message || "Помилка!", "error"); }
}

async function acceptTrade(docId)  { try { await updateTradeStatus(docId,"confirmed"); await renderTrades(); showToast("✅ Трейд підтверджено!", "success"); } catch(e){showToast("Помилка: "+e.message,"error");} }
async function declineTrade(docId) { try { await updateTradeStatus(docId,"cancelled"); await renderTrades(); showToast("❌ Трейд відхилено","error");         } catch(e){showToast("Помилка: "+e.message,"error");} }
async function cancelTrade(docId)  { try { await updateTradeStatus(docId,"cancelled"); await renderTrades(); showToast("Трейд скасовано","success");           } catch(e){showToast("Помилка: "+e.message,"error");} }
...
function renderFriends() {
  ...
  list.innerHTML = friends.map(u =>
    ...
    '<div class="friend-actions"><button class="btn-sm btn-cancel" onclick="removeFriend(\'' + u + '\')">Видалити</button></div>' +
    '</div>'
  ).join("");
}
...
async function addFriend() {
  ...
  // ФІКС: перевіряємо існування юзера в Firebase
  try {
...
function renderCraft() {
  ...
  slots.innerHTML =
    craftSelected.map((item, i) =>
      ...
      '<div>' + item.name + '</div><div style="font-size:10px;color:#aaa;">✕ прибрати</div></div>'
    ).join("") +
...
async function doCraft() {
  ...
  craftSelected.forEach(sel => { const i = gameState.inventory.findIndex(x => x.id === sel.id); if (i!==-1) gameState.inventory.splice(i,1); });
  const newItem = { id:generateId(), type:"item", itemId:"crafted_"+newRarity, name:"Крафтовий предмет", img:"img/items/placeholder.png", rarity:newRarity, quality:rollQuality(), premium:Math.random()<0.03, fromCase:"craft", obtainedAt:Date.now() };
  gameState.inventory.push(newItem);
  craftSelected = [];
  await saveData(); renderCraft();
  const r = RARITIES[newRarity];
  openModal('<div style="text-align:center;border:2px solid ' + r.color + ';border-radius:12px;padding:24px;background:' + r.color + '22;"><div style="font-size:18px;margin-bottom:8px;">⚗️ Крафт успішний!</div><img src="' + newItem.img + '" style="width:80px;height:80px;object-fit:contain;margin:12px 0;"><div style="font-weight:700;">' + newItem.name + '</div><div style="color:' + r.color + '">' + r.name + '</div><div style="color:#aaa;font-size:12px;">' + newItem.quality + '</div><button class="btn-primary" onclick="closeModal()" style="margin-top:16px;width:100%;">Забрати!</button></div>');
}
...
function openAdmin() {
  ...
  openModal(
    ...
    '<p style="color:var(--text-muted);font-size:13px;margin-bottom:12px;">Введіть пароль адміністратора:</p>' +
...
async function renderAdmin() {
  ...
  if (!page) return;
  page.innerHTML = '<div class="page-header"><button class="btn-back" onclick="navigate(\'main\')">← Назад</button><h1 class="page-title">🔐 Адмін панель</h1></div><div class="empty-state">⏳ Завантаження гравців...</div>';
  injectAdminStyles();
  try {
    const snap = await getDocs(collection(db, "users"));
    _adminUsers = snap.docs.map(d => ({ uid: d.id, ...d.data() }));
  } catch (e) {
    page.innerHTML += '<div class="empty-state">❌ Помилка: ' + e.message + '</div>';
    return;
  }
  const usersHtml = _adminUsers.map(u =>
    '<div class="admin-user-row">' +
      ...
      '<div class="admin-user-actions">' +
        '<button class="btn-sm btn-primary" onclick="adminGiveBalance(\'' + u.uid + '\')">💰 Баланс</button>' +
...
async function adminGiveBalance(uid) {
  const u = _adminUsers.find(x => x.uid === uid);
  openModal(
    '<h2 class="modal-title">💰 Видати баланс</h2>' +
    ...
    '<div style="display:flex;gap:8px;">' +
    '<button class="btn-primary" style="flex:1;" onclick="adminConfirmBalance(\'' + uid + '\')">Видати</button>' +
    '<button class="btn-secondary" style="flex:1;" onclick="closeModal()">Скасувати</button></div>'
  );
}

async function adminConfirmBalance(uid) {
  ...
  try {
    ...
    if (uid === currentUser?.uid) { gameState.balance = cur + amount; updateBalanceDisplay(); }
    closeModal();
    showToast("✅ Видано " + amount + " нікусів!", "success");
    await renderAdmin();
  } catch (e) { showToast("Помилка: " + e.message, "error"); }
}
...
async function adminGiveItem(uid) {
  ...
  openModal(
    ...
    '<input type="text" id="admin-item-name" class="form-input" value="Адмін предмет" placeholder="Назва предмета" style="width:100%;box-sizing:border-box;margin:8px 0;">' +
...
async function adminConfirmItem(uid) {
  ...
  try {
    ...
    closeModal();
    showToast('✅ Предмет "' + name + '" видано!', "success");
    await renderAdmin();
  } catch (e) { showToast("Помилка: " + e.message, "error"); }
}
...
async function adminConfirmCase(uid) {
  ...
  try {
    ...
    if (uid === currentUser?.uid) { gameState.inventory = inv; renderInventory(); }
    closeModal();
    showToast('✅ Кейс "' + c.name + '" видано!', "success");
    await renderAdmin();
  } catch (e) { showToast("Помилка: " + e.message, "error"); }
}
  ...
  onAuthStateChanged(auth, (firebaseUser) => {
    if (firebaseUser) {
      ...
      _unsubProfile = onSnapshot(doc(db, "users", firebaseUser.uid), (snap) => {
        ...
        // НЕ перезаписуємо якщо зараз зберігаємо — захист від race condition
        if (_isSaving) return;
