...
async function initGlobalCases() {
  try {
    ...
    if (!snap.exists()) {
      ...
      // Завантажуємо глобальні залишки
...
function subscribeGlobalCases() {
  return onSnapshot(doc(db, "globalData", "cases"), (snap) => {
    ...
    // Оновити магазин якщо відкритий
...
async function showTradeModal(itemId) {
  ...
  openModal(
    ...
    '<select id="trade-target" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);background:var(--bg-input);color:var(--text);font-size:14px;box-sizing:border-box;margin-bottom:14px;">' +
    ...
    '<input type="text" id="trade-want-name" placeholder="Назва предмета..." style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--border);background:var(--bg-input);color:var(--text);font-size:14px;margin-bottom:16px;box-sizing:border-box;">' +
    '<div style="display:flex;gap:8px;">' +
...
async function renderProfile(targetUsername) {
  ...
  // Завантажуємо дані — або свій профіль, або чужий
...
// ── ПЕРЕГЛЯД ПРОФІЛЮ ГРАВЦЯ (повний) ───────

async function viewFriendProfile(username) {
  // Відкриваємо сторінку профілю з даними цього гравця
  showAllPages(false);
  const page = document.getElementById("page-profile");
...
// Вибирає значення з масиву зважено (більше однакових = більший шанс)
...
function calcCraftAttributes(items) {
  // Збираємо всі значення якостей, колекцій та стану (преміум)
  ...
  // Стан (преміум) — 1/5 тих що преміум проти тих що ні
...
function renderCraft() {
  ...
  // Визначаємо рідкість вибраних (повинні бути однакові)
  ...
  if (picker) {
    ...
    // Якщо вже є вибрані — показуємо тільки ту ж рідкість
...
async function doCraft() {
  ...
  // Видаляємо предмети незалежно від результату
  ...
  // Успіх — визначаємо атрибути
  const { quality, collection, premium } = calcCraftAttributes(craftSelected);

  // Знаходимо підходящий предмет для нової рідкості
