...
function buildMellWheelSegments() {
  ...
  return MELL_SPIN_TABLE.map((entry, i) => {
    ...
    // Чергуємо яскравість для кращого розрізнення сегментів
    const dimmed = i % 2 === 0 ? color + "dd" : color + "88";
    return { ...entry, angle, color: dimmed };
  });
}
...
function drawWheel(canvas, segments, rotation) {
  ...
  // Зовнішнє обідце з золотими крапками
  const dotCount = 20;
...
async function openMellSpinModal() {
  injectMellSpinStyles();
  const coins = gameState.mellCoins || 0;

  const tableHtml = MELL_SPIN_TABLE.map(entry => {
    const totalW = MELL_SPIN_TABLE.reduce((s, x) => s + x.weight, 0);
    const pct    = ((entry.weight / totalW) * 100).toFixed(1);
    return `<div class="mell-spin-row">
      <span class="mell-spin-label">${entry.label}</span>
      <span class="mell-spin-pct">${pct}%</span>
    </div>`;
  }).join("");

  openModal(`
    <h2 class="modal-title">🎰 МеллКоінс Спін</h2>
    <div style="text-align:center;margin-bottom:12px;">
      <span style="font-size:22px;font-weight:800;color:#ffe066;">${coins} 🪙</span>
      <span style="color:var(--text-muted);font-size:13px;margin-left:8px;">МеллКоінс</span>
    </div>

    <div style="display:flex;flex-direction:column;align-items:center;margin-bottom:14px;">
      <canvas id="mell-arrow-canvas" width="30" height="34"
        style="display:block;margin-bottom:0;z-index:2;"></canvas>
      <canvas id="mell-wheel-canvas" width="280" height="280"
        style="display:block;max-width:min(280px,85vw);height:auto;border-radius:50%;
               box-shadow:0 0 30px rgba(255,215,0,0.3),0 4px 20px rgba(0,0,0,0.5);"></canvas>
    </div>

    <div id="mell-spin-result" style="display:none;text-align:center;margin-bottom:12px;"></div>

    <div id="mell-spin-btns" style="display:flex;gap:8px;">
      <button class="btn-primary" style="flex:1;" id="mell-spin-go-btn"
        onclick="doMellSpinAction()" ${coins < MELL_SPIN_COST ? "disabled" : ""}>
        🎰 Крутити (${MELL_SPIN_COST} 🪙)
      </button>
      <button class="btn-secondary" style="flex:1;" onclick="closeModal()">Закрити</button>
    </div>
    <button id="mell-spin-again-btn" class="btn-primary btn-full"
      style="display:none;margin-top:8px;" onclick="openMellSpinModal()">
      🎰 Ще раз
    </button>

    <details style="margin-top:12px;">
      <summary style="font-size:12px;font-weight:700;color:var(--text-muted);cursor:pointer;padding:4px 0;">
        📊 Таблиця шансів
      </summary>
      <div style="background:var(--bg-card2);border:1px solid var(--border);border-radius:10px;padding:10px;margin-top:6px;">
        ${tableHtml}
      </div>
    </details>
  `, true);
  ...
  requestAnimationFrame(() => {
    ...
    if (!wheelCanvas || !arrowCanvas) return;
    const segments = buildMellWheelSegments();
    window._mellWheelSegments = segments;
    drawArrow(arrowCanvas);
    drawWheel(wheelCanvas, segments, 0);
  });
}

async function doMellSpinAction() {
  const goBtn    = document.getElementById("mell-spin-go-btn");
  const btnsEl   = document.getElementById("mell-spin-btns");
  const againBtn = document.getElementById("mell-spin-again-btn");
  const resultEl = document.getElementById("mell-spin-result");
  const wheelCanvas = document.getElementById("mell-wheel-canvas");

  if (!wheelCanvas) { showToast("Помилка UI колеса", "error"); return; }
  if (goBtn) goBtn.disabled = true;

  const segments = window._mellWheelSegments || buildMellWheelSegments();

  // Запускаємо Firestore паралельно з анімацією
  const spinPromise = doMellSpin(currentUser.uid, gameState).catch(e => { throw e; });

  // Попередньо обчислюємо "випадковий" приз для анімації (колесо крутиться довільно)
  // Реальний результат прийде з Firestore і ми підганяємо фінальну позицію під нього
  const tempIdx = Math.floor(Math.random() * segments.length);
  let segStart0 = 0;
  for (let i = 0; i < tempIdx; i++) segStart0 += segments[i].angle;
  const tempMid = segStart0 + segments[tempIdx].angle / 2;
  const tempTarget = -Math.PI / 2 - tempMid;
  const fullSpins  = (6 + Math.floor(Math.random() * 3)) * Math.PI * 2;
  const animEnd    = tempTarget + fullSpins;
  ...
  function animate(now) {
    if (!document.getElementById("mell-wheel-canvas")) return; // модаль закрита
    const elapsed = now - startTime;
    const t       = Math.min(elapsed / duration, 1);
    const current = easeOut(t) * animEnd;
    drawWheel(wheelCanvas, segments, current);
    if (t < 1) {
      requestAnimationFrame(animate);
    } else {
      animDone = true;
      if (prizeFromServer !== null) {
        _finishSpin(prizeFromServer, segments, resultEl, btnsEl, againBtn, wheelCanvas);
      }
    }
  }
  ...
  // Чекаємо результат з сервера
  try {
    const prize = await spinPromise;

    // Оновлюємо gameState одразу
    const snap = await getDoc(doc(db, "users", currentUser.uid));
    if (snap.exists()) {
      gameState.mellCoins = snap.data().mellCoins || 0;
      gameState.inventory = snap.data().inventory || [];
    ...
    if (animDone) {
      _finishSpin(prize, segments, resultEl, btnsEl, againBtn, wheelCanvas);
    }
  } catch(e) {
    showToast("Помилка: " + e.message, "error");
...
function _finishSpin(prize, segments, resultEl, btnsEl, againBtn, wheelCanvas) {
  // Доводимо колесо до правильного сегменту
  ...
  const finalAngle = -Math.PI / 2 - segMid + (Math.random() - 0.5) * segments[targetIdx].angle * 0.4;

  // Домальовуємо колесо у фінальній позиції
  if (wheelCanvas) drawWheel(wheelCanvas, segments, finalAngle);
  ...
  if (resultEl) {
    ...
    resultEl.innerHTML = `
      <div style="border:2px solid ${color};border-radius:14px;padding:16px;
                  background:${color}18;display:inline-block;min-width:200px;
                  animation:popupIn .3s ease-out;">
        <div style="color:#aaa;font-size:13px;margin-bottom:6px;">🎉 Ти виграв!</div>
        <div style="font-size:17px;font-weight:800;color:${color};">${prize.label}</div>
        <div style="color:var(--text-muted);font-size:12px;margin-top:8px;">
          🪙 Залишилось: <b style="color:#ffe066;">${gameState.mellCoins}</b>
        </div>
      </div>`;
  }
  ...
  if (againBtn) {
    ...
    againBtn.disabled = (gameState.mellCoins || 0) < MELL_SPIN_COST;
  }

  const mellCoinsDisplay = document.getElementById("mellcoins-display");
  if (mellCoinsDisplay) mellCoinsDisplay.textContent = gameState.mellCoins;
}
