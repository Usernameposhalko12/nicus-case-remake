...
const COLLECTIONS = {
 summer26: {
    name: "Літо26",
    cases: ["summer26", "summer26box", "summergift26", "autumnCollection"],
  },
  AutumnLeaves: {
    name: "AutumnLeaves",
    cases: ["autumnLeaves"],
...
const CASES = {
  ...
  summer26box: {
    ...
    img: "img/cases/summer26box.png", price: 25, total: 3000, remaining: 3000,
  ...
  autumnCollection: {
    ...
    // 100н — колекційний, тільки exceptional+, без common
    chances: {
      common:      0,
      exceptional: 0.40,
      epic:        0.35,
    ...
    items: [
      { id: "item_c1", name: "Лут 111", img: "img/items/placeholder.png", rarity: "exceptional", collection: "summer26"},
      { id: "item_c2", name: "Лут 222", img: "img/items/placeholder.png", rarity: "epic", collection: "summer26"},
      { id: "item_c3", name: "Лут 333", img: "img/items/placeholder.png", rarity: "legendary", collection: "summer26"},
      { id: "item_c4", name: "Лут 444", img: "img/items/placeholder.png", rarity: "secret", collection: "summer26"},
    ]
  },
  ...
  autumnLeaves: {
    ...
    items: [
      { id: "item_e1", name: "Лут 1", img: "img/items/placeholder.png", rarity: "legendary", collection: "autumnLeaves"},
      { id: "item_e2", name: "Лут 2", img: "img/items/placeholder.png", rarity: "secret", collection: "autumnLeaves"},
      { id: "item_e3", name: "Лут 3", img: "img/items/placeholder.png", rarity: "special", collection: "autumnLeaves"},
    ]
  },
  ...
  autumnVibe: {
    ...
    items: [
      { id: "item_f1", name: "Лут 1", img: "img/items/placeholder.png", rarity: "legendary", collection: "autumnVibe"},
      { id: "item_f2", name: "Лут 2", img: "img/items/placeholder.png", rarity: "secret", collection: "autumnVibe"},
      { id: "item_f3", name: "Лут 3", img: "img/items/placeholder.png", rarity: "special", collection: "autumnVibe"},
    ]
  },
  ...
  leaffall: {
    ...
    items: [
      { id: "item_g1", name: "Лут 1", img: "img/items/placeholder.png", rarity: "legendary", collection: "leaffall"},
      { id: "item_g2", name: "Лут 2", img: "img/items/placeholder.png", rarity: "secret", collection: "leaffall"},
      { id: "item_g3", name: "Лут 3", img: "img/items/placeholder.png", rarity: "special", collection: "leaffall"},
    ]
  },
};
...
if (clanBadge) clanBadge.textContent = gameState.clan || "Без клану";
...
async function renderMarketRequests() {
  const grid = document.getElementById("market-grid");
  if (!grid) return;
  grid.innerHTML = '<div class="mkt-empty"><div class="mkt-empty-icon">⏳</div><div class="mkt-empty-text">Завантаження...</div></div>';

  try {
    const { collection: col, query: q, where, getDocs, orderBy } = await import("https://www.gstatic.com/firebasejs/10.7.0/firebase-firestore.js");

    const [sentSnap, recvSnap] = await Promise.all([
      getDocs(q(col(db, "marketRequests"), where("buyerId",  "==", currentUser.uid))),
      getDocs(q(col(db, "marketRequests"), where("sellerId", "==", currentUser.uid))),
    ]);

    const all = [];
    sentSnap.docs.forEach(d => all.push({ id: d.id, ...d.data(), _role: "buyer" }));
    recvSnap.docs.forEach(d => {
      if (!all.find(x => x.id === d.id)) all.push({ id: d.id, ...d.data(), _role: "seller" });
    });
    ...
    grid.innerHTML = '<div class="mkt-empty"><div class="mkt-empty-text">❌ ' + e.message + '</div></div>';
  }
}
...
function switchClanTab(tab) {
  const tabs = ["members","vault","chat","requests"];
  tabs.forEach(t => {
...
async function doVaultDepositItem() {
  ...
  try {
    ...
    closeModal();
    showToast("✅ Предмет у общаку!", "success");
    // Оновлюємо вкладку общаку без скидання табу
...
async function refreshVaultSection() {
  ...
  try {
    ...
    if (!clan) return;
    const vault = clan.vault || { balance: 0, inventory: [] };
    const balEl = vaultSection.querySelector("[data-vault-balance]");
    // Повний ребілд тільки вмісту vault секції
    vaultSection.innerHTML =
      '<div style="display:flex;justify-content:space-between;align-items:center;padding:14px;background:var(--bg-card2);border:1px solid var(--border);border-radius:10px;margin-bottom:12px;">' +
        '<span>💰 Баланс общаку:</span>' +
        '<span style="font-weight:700;color:var(--gold-light);font-size:18px;" data-vault-balance>' + (vault.balance||0) + ' нікусів</span>' +
      '</div>' +
      '<div style="display:flex;gap:8px;margin-bottom:14px;">' +
        '<button class="btn-primary" style="flex:1;" onclick="vaultDepositBalance()">+ Поповнити</button>' +
        '<button class="btn-secondary" style="flex:1;" onclick="vaultWithdrawBalance()">- Забрати</button>' +
      ...
      (vault.inventory && vault.inventory.length
        ...
        : '<div class="empty-state" style="padding:20px;">Общак порожній</div>') +
      '<div style="margin-top:12px;"><button class="btn-secondary btn-full" onclick="vaultDepositItem()">🎁 Покласти предмет</button></div>';
  } catch (e) { console.error("refreshVaultSection:", e); }
}
...
async function adminGiveItem(uid) {
  ...
  // Збираємо всі унікальні предмети з усіх кейсів
  const allItems = [];
  const seen = new Set();
  Object.values(CASES).forEach(c => {
...
async function adminConfirmItem(uid) {
  ...
  if (selectedId) {
    ...
    itemToGive = {
      ...
      quality: rollQuality(), premium: Math.random() < 0.05,
      fromCase: fromCaseId || "admin", obtainedAt: Date.now()
    };
  } else {
    ...
    itemToGive = {
      ...
      quality: rollQuality(), premium: false,
      fromCase: "admin", obtainedAt: Date.now()
    };
  }
...
async function adminConfirmResetCases() {
  ...
  try {
    await updateDoc(doc(db, "globalData", "cases"), updates);
