  ...
  const tryRender = async (attempt) => {
    try {
      ...
      if (attempt < 3) {
        ...
        // Перевіряємо що юзер все ще на цій сторінці
        ...
        // Показуємо кнопку ретрай в контейнері
        const containers = {
          clan: 'clan-section',
          trades: 'trades-list',
...
'<div style="font-weight:700;font-size:13px;margin-bottom:8px;">📋 Останні дії:</div>' +
((clan.logs||[]).length
  ...
  : '<div style="color:var(--text-muted);font-size:13px;text-align:center;padding:10px;">Дій ще немає</div>') +
'</div>'
