...
import {
  auth, registerUser, loginUser, saveUserData, logoutUser,
...
async function confirmSell(invIdx) {
  const item  = gameState.inventory[invIdx];
  const price = parseInt(document.getElementById("sell-price").value);
  if (!price || price < 1) { showToast("Введіть коректну ціну!", "error"); return; }
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // Якщо користувач вже залогінений (Firebase session)
