...
async function buyFromMarketItem(docId) {
  ...
  const btn = document.querySelector(`button[onclick="buyFromMarketItem('${docId}')"]`);
...
async function showTradeModal(itemId) {
  // itemId може бути null — відкриваємо порожній трейд
  _tradeMySelected    = [];
  _tradeTheirSelected = [];
  ...
  tradeRenderMyPicker();
  tradeRenderMyBasket();
  // Завантажуємо їхній інвентар одразу якщо є гравці
...
function injectThemeButton() {
  ...
  // Також кнопка в налаштуваннях на головній
  const qs = document.getElementById("page-main");
  // не додаємо в DOM — кнопка в навбарі достатня
}
