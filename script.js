...
let _isSaving      = false;
let _lockedItems   = new Set(); // заблоковані предмети (set of item ids)
...
async function navigate(page) {
  ...
  // підписуємось на ринок тільки коли відкрита вкладка
  if (page === 'market') {
...
function openCase(invId) {
  ...
  // зберігаємо dropped для fast skip
...
function renderInventory() {
  ...
  // Відмальовуємо фільтри по колекціях
  ...
  if (!items.length) {
    grid.innerHTML = '<div class="empty-state">🍂 Порожньо</div>'; return;
  }
...
// Трейд — вибір предмета зі свого інвентаря
...
// ГІФТ — відправка без очікування зворотного предмета
...
async function sendGift() {
  ...
  const itemSection    = document.getElementById("gift-item-section");
...
async function renderProfile() {
  ...
  s("profile-items",    (gameState.inventory||[]).filter(i=>i.type==="item").length);
  s("profile-cases",    (gameState.inventory||[]).filter(i=>i.type==="case").length);
  s("profile-lastseen", gameState.lastSeen ? new Date(gameState.lastSeen).toLocaleDateString("uk-UA") : "—");
  s("profile-xp",       gameState.xp||0);

  // Показуємо назву клану, а не ID
  if (gameState.clan) {
    try {
      const clan = await getClan(gameState.clan);
      s("profile-clan", clan ? clan.name : "Клан");
    } catch (e) { s("profile-clan", "Клан"); }
  } else {
    s("profile-clan", "Без клану");
  }

  // Статистика рідкостей
  const inv = (gameState.inventory || []).filter(i => i.type === "item");
  const rarStats = document.getElementById("profile-rar-stats");
  if (rarStats) {
    rarStats.innerHTML = Object.entries(RARITIES).map(([key, r]) => {
      const count = inv.filter(i => i.rarity === key).length;
      return '<div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border);">' +
        '<span style="color:' + r.color + ';font-size:13px;">' + r.name + '</span>' +
