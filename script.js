...
import {
  ...
  BOT_USERNAMES, BOT_BUY_INTERVAL_MS, runMarketBotsTick,
...
async function adminBotsPanel() {
  ...
  const botUsers = BOT_USERNAMES.map(name => {
    ...
    return { name, uid: u.uid || ("bot_" + name), balance: u.balance || 0, inv: (u.inventory || []).length };
  });

  const botRows = botUsers.map(b => {
    const isOff = disabledBots.includes(b.name);
    return `<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;padding:10px 12px;background:var(--bg-card2);border-radius:8px;margin-bottom:8px;flex-wrap:wrap;">
      <div>
        <span style="font-weight:700;">${isOff ? '⏸️' : '🤖'} ${b.name}</span>
        <span style="color:var(--text-muted);margin-left:8px;font-size:13px;">💰 ${b.balance} | 🎒 ${b.inv} пр.</span>
  ...
  openModal(`
    ...
    <div style="background:var(--bg-card2);border-radius:10px;padding:12px;margin-bottom:14px;">
      ...
      <div style="font-size:13px;color:var(--text-muted);margin-bottom:10px;">⏱️ До наступної покупки: <b>${nextBuyMs <= 0 ? 'зараз/при наступному тіку' : _botTimeAgoOrIn(nextBuyMs)}</b></div>
