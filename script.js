...
async function doMellCraftAction() {
  ...
  try {
    ...
    const fallback = Object.values(CASES).flatMap(c => c.items.filter(i => i.rarity === nextRarity));
    const picked = (pool.length ? pool : fallback)[Math.floor(Math.random() * (pool.length || fallback.length))];
    ...
    const newItem = {
      ...
      source: "mellcraft", obtainedBy: currentUser?.username || "?", obtainedAt: Date.now(),
    };
    gameState.inventory.push(newItem);

    // Нараховуємо МеллКоінс + ХП
    ...
    <div class="mell-two-col">
      <div class="mell-section">
        ...
        <button class="mell-spin-action-btn" id="mell-spin-inline-btn"
          onclick="_startMellWheelSpin(_mellSpring.power || 0.5)"
...
function openMellSpinModal() {
  ...
  const rows = MELL_SPIN_TABLE.map(e => {
    const pct = ((e.weight / totalW) * 100).toFixed(1);
    return `<div class="mell-spin-row">
      <span class="mell-spin-label">${e.label}</span>
