...
// Зупиняємо mellcraft підписку при виході
...
function injectMellCraftStyles() {
  ...
  s.textContent = `
    .mellcraft-header { display:flex;align-items:center;gap:12px;padding:14px;background:var(--bg-card);border:1px solid var(--border);border-radius:12px;margin-bottom:14px;flex-wrap:wrap; }
    .mellcraft-stat { display:flex;align-items:center;gap:8px;flex:1; }
    ...
    .mell-spin-btn { padding:10px 14px;border-radius:10px;border:none;background:linear-gradient(135deg,#ffe066,#ffaa00);color:#111;font-size:13px;font-weight:800;cursor:pointer;text-align:center;line-height:1.3; }
    .mellhp-bar-wrap { background:var(--bg-card);border:1px solid var(--border);border-radius:12px;padding:14px;margin-bottom:14px; }
    .mellhp-bar { position:relative;height:18px;background:var(--bg-input);border-radius:9px;overflow:visible;margin:6px 0; }
    .mellhp-fill { height:100%;background:linear-gradient(90deg,#5ddb8a,#00c9a7);border-radius:9px;transition:width 0.5s; }
    .mellhp-milestone { position:absolute;top:-8px;transform:translateX(-50%);font-size:16px;cursor:pointer; }
    .mellhp-milestone.done { opacity:0.5; }
    .mellcraft-section { background:var(--bg-card);border:1px solid var(--border);border-radius:12px;padding:14px; }
    .craft-slots { display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px; }
    .craft-slot { width:70px;height:85px;border:2px dashed var(--border);border-radius:10px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;cursor:pointer;background:var(--bg-card2); }
    .craft-slot img { width:44px;height:44px;object-fit:contain; }
    .craft-slot.empty { color:var(--text-muted);font-size:22px; }
    .craft-picker { display:flex;flex-direction:column;gap:6px;max-height:280px;overflow-y:auto; }
    .craft-pick-item { display:flex;align-items:center;gap:8px;padding:8px 10px;background:var(--bg-card2);border-radius:8px;cursor:pointer;border-left:3px solid transparent; }
...
async function renderMellCraft() {
  ...
  const freshGlobal = await getMellCraftGlobal();
  _mellGlobalData = freshGlobal;

  const coins  = gameState.mellCoins   || 0;
  const contrib = gameState.mellHpContrib || 0;
  const totalHp = _mellGlobalData.totalHp || 0;

  // Наступний milestone
  const nextMilestone = MELL_HP_REWARDS.find(m => m.hp > totalHp) || null;
  const progressToNext = nextMilestone
    ? Math.min(100, Math.round(((totalHp - (MELL_HP_REWARDS[MELL_HP_REWARDS.findIndex(m => m.hp === nextMilestone.hp) - 1]?.hp || 0)) /
       (nextMilestone.hp - (MELL_HP_REWARDS[MELL_HP_REWARDS.findIndex(m => m.hp === nextMilestone.hp) - 1]?.hp || 0))) * 100))
    : 100;

  page.innerHTML = `
    <div class="page-header">
      <button class="btn-back" onclick="navigate('craft')">← Назад</button>
      <h1 class="page-title">🎭 МеллКрафт</h1>
    </div>

    <!-- МеллКоінс + МеллХП хедер -->
    <div class="mellcraft-header">
      <div class="mellcraft-stat">
        <span class="mellcraft-stat-icon">🪙</span>
        <div>
          <div class="mellcraft-stat-val" id="mellcoins-display">${coins}</div>
      ...
      <div class="mellcraft-stat">
        <span class="mellcraft-stat-icon">⚡</span>
        <div>
          <div class="mellcraft-stat-val">${contrib}</div>
          <div class="mellcraft-stat-label">Мій внесок ХП</div>
        </div>
      </div>
      <button class="mell-spin-btn" onclick="openMellSpinModal()">
        🎰 Спін<br><span style="font-size:10px;opacity:.8;">${MELL_SPIN_COST} МеллКоінс</span>
      </button>
    </div>

    <!-- Глобальна смуга МеллХП -->
    <div class="mellhp-bar-wrap" id="mellhp-bar-wrap">
      ${_buildMellHPBarHTML(totalHp)}
    </div>

    <!-- Крафт секція -->
    <div class="mellcraft-section">
      <div style="font-weight:700;font-size:15px;color:var(--text);margin-bottom:10px;">⚗️ Крафт МеллСтрой предметів</div>
      <p style="color:var(--text-muted);font-size:13px;margin-bottom:12px;">
        Вибери 5 предметів <b>з колекції Меллстрой</b> однієї рідкості.<br>
        За кожен крафт отримуєш <b style="color:#ffe066;">МеллКоінс</b> + <b style="color:#5ddb8a;">+${MELL_CRAFT_HP} МеллХП</b> глобально.
      </p>

      <div id="mellcraft-slots" class="craft-slots"></div>
      <div id="mellcraft-info" style="margin:10px 0;"></div>
      <button id="mellcraft-btn" class="btn-primary btn-full" onclick="doMellCraftAction()" disabled>⚗️ Крафтити</button>

      <div style="font-size:13px;color:var(--text-muted);margin:14px 0 8px;font-weight:700;">🎒 Меллстрой предмети в інвентарі:</div>
...
function _buildMellHPBarHTML(totalHp) {
  const maxHp = 1000;
  const pct   = Math.min(100, Math.round((totalHp / maxHp) * 100));
  const claimed = _mellGlobalData.claimedLevels || [];

  const milestonesHtml = MELL_HP_REWARDS.filter(m => m.hp > 0).map(m => {
    const pos = Math.round((m.hp / maxHp) * 100);
    const done = claimed.includes(m.hp) || totalHp >= m.hp;
    return `<div class="mellhp-milestone${done?" done":""}" style="left:${pos}%;" title="${m.reward.label}">
      ${done ? "✅" : (m.reward.type === "medal" ? "🏅" : "📦")}
    </div>`;
  }).join("");
  ...
  return `
    <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text-muted);margin-bottom:4px;">
      ...
      <span><b style="color:#5ddb8a;">${totalHp}</b> / ${maxHp}</span>
    </div>
    <div class="mellhp-bar">
      <div class="mellhp-fill" style="width:${pct}%"></div>
      ${milestonesHtml}
    </div>
    <div style="font-size:11px;color:var(--text-muted);margin-top:6px;text-align:center;">
      ${totalHp >= maxHp
        ? "🏅 Глобальний шлях завершено!"
        : "Наступна нагорода: " + (MELL_HP_REWARDS.find(m => m.hp > totalHp)?.reward.label || "—") +
          " при " + (MELL_HP_REWARDS.find(m => m.hp > totalHp)?.hp || "?") + " ХП"}
    </div>
  `;
}

function _updateMellHPBar() {
  const mellPage = document.getElementById("page-mellcraft");
  if (!mellPage || mellPage.style.display === "none") return;
  const wrap = document.getElementById("mellhp-bar-wrap");
  if (!wrap) return;
  wrap.innerHTML = _buildMellHPBarHTML(_mellGlobalData.totalHp || 0);
}
...
function mellRenderSlots() {
  ...
  const rarities = [...new Set(mellCraftSelected.map(i => i.rarity).filter(Boolean))];
  const sameRarity = rarities.length === 1;
  const currentRarity = sameRarity ? rarities[0] : null;
  ...
  slots.innerHTML =
    ...
    Array(Math.max(0, 5 - mellCraftSelected.length)).fill('<div class="craft-slot empty">+</div>').join("");

  const canCraft = mellCraftSelected.length === 5 && sameRarity;
  if (btn) btn.disabled = !canCraft;

  if (info) {
    const coinsInfo = currentRarity && mellCraftSelected.length === 5
      ? ` | +<b style="color:#ffe066;">${Math.min(50, MELL_CRAFT_COINS[currentRarity]?.base || 5)} МеллКоінс</b>`
      : "";
    if (!mellCraftSelected.length) {
      info.innerHTML = `<div style="color:var(--text-muted);font-size:13px;text-align:center;padding:8px;">Вибери 5 предметів Меллстрой однієї рідкості</div>`;
    } else if (mellCraftSelected.length < 5 || !sameRarity) {
      info.innerHTML = `<div style="color:${!sameRarity&&mellCraftSelected.length>1?"#e74c3c":"var(--text-muted)"};font-size:13px;text-align:center;padding:8px;">
        ${!sameRarity&&mellCraftSelected.length>1?"⚠️ Тільки однакова рідкість!":"Вибрано " + mellCraftSelected.length + " / 5"}
      </div>`;
    } else {
      info.innerHTML = `
        <div style="background:var(--bg-card2);border:1px solid var(--border);border-radius:10px;padding:10px;text-align:center;font-size:13px;">
          Результат (50%): <b style="color:${nextR?.color||'#888'}">${nextR?.name||nextRarity}</b>
          ${coinsInfo} | <b style="color:#5ddb8a;">+${MELL_CRAFT_HP} МеллХП</b>
          <div style="font-size:11px;color:#e74c3c;margin-top:4px;">⚠️ При невдачі — предмети зникають!</div>
        </div>`;
    }
  }
}
...
function mellRenderPicker() {
  ...
  const rarities = [...new Set(mellCraftSelected.map(i => i.rarity).filter(Boolean))];
  const currentRarity = rarities.length === 1 ? rarities[0] : null;

  const available = (gameState.inventory || []).filter(i =>
    i.type === "item" &&
    !_lockedItems.has(i.id) &&
    !mellCraftSelected.find(s => s.id === i.id) &&
    MELLCRAFT_MELL_ITEMS.includes(i.itemId || "")
  );

  const filtered = currentRarity && mellCraftSelected.length > 0
    ? available.filter(i => i.rarity === currentRarity)
    : available;

  if (!filtered.length) {
    picker.innerHTML = `<div class="empty-state">Немає предметів Меллстрой${currentRarity ? ' рідкості "'+RARITIES[currentRarity]?.name+'"' : ''}</div>`;
    return;
  }
  ...
  picker.innerHTML = filtered.map(item => {
    const r = item.rarity ? RARITIES[item.rarity] : null;
    const dim = currentRarity && item.rarity !== currentRarity;
    return `<div class="craft-pick-item${dim?" craft-dim":""}"
      onclick="${dim?"":"mellAddItem('"+item.id+"')"}"
      style="border-left:3px solid ${r?r.color:'#888'}">
      <img src="${item.img||'img/placeholder.png'}" onerror="this.src='img/placeholder.png'">
      <span>${item.name}</span>
      ${r?`<span style="color:${r.color};font-size:11px;">${r.name}</span>`:''}
      ${item.premium?'<span style="color:#FFD700;font-size:10px;">⭐</span>':''}
    </div>`;
  }).join("");
}

function mellAddItem(itemId) {
  if (mellCraftSelected.length >= 5) return;
  const item = (gameState.inventory || []).find(i => i.id === itemId);
  if (!item) return;
  if (!MELLCRAFT_MELL_ITEMS.includes(item.itemId || "")) {
    showToast("Тільки предмети з колекції Меллстрой!", "error"); return;
  }
  if (mellCraftSelected.length > 0 && mellCraftSelected[0].rarity !== item.rarity) {
    showToast("Тільки однакова рідкість!", "error"); return;
  }
  mellCraftSelected.push(item);
  mellRenderSlots();
  mellRenderPicker();
}

function mellRemoveItem(index) {
  mellCraftSelected.splice(index, 1);
  mellRenderSlots();
  mellRenderPicker();
}
...
async function doMellCraftAction() {
  ...
  // Видаляємо предмети з інвентаря
  mellCraftSelected.forEach(sel => {
    const i = gameState.inventory.findIndex(x => x.id === sel.id);
    if (i !== -1) gameState.inventory.splice(i, 1);
  });
  ...
  if (!success) {
    ...
    openModal(`
      <div style="text-align:center;border:2px solid #e74c3c;border-radius:12px;padding:24px;background:rgba(231,76,60,0.1);">
        <div style="font-size:48px;margin-bottom:12px;">💥</div>
        <div style="font-size:20px;font-weight:700;color:#e74c3c;margin-bottom:8px;">МеллКрафт провалився!</div>
        <div style="color:var(--text-muted);font-size:14px;margin-bottom:16px;">Всі 5 предметів зникли...</div>
        <button class="btn-primary" onclick="closeModal()" style="width:100%;">Зрозуміло</button>
      </div>`
    );
    return;
  }

  // Крафт успішний — берем предмет з Меллстрой колекції
  const { quality, collection, premium } = calcCraftAttributes(mellCraftSelected);
  const pool = Object.values(CASES).flatMap(c => c.items.filter(i =>
    i.collection === "mellstroy" && i.rarity === nextRarity
  ));
  const picked = pool.length ? pool[Math.floor(Math.random() * pool.length)] : null;
  ...
  const craftedItem = {
    id: generateId(), type: "item",
    itemId: picked ? picked.id : "crafted_" + nextRarity,
    name: picked ? picked.name : "МеллКрафт " + (RARITIES[nextRarity]?.name || nextRarity),
    img: picked ? picked.img : "img/items/m1.png",
  ...
  // Надсилаємо в Firestore (МеллКоінс + глобальний ХП)
  let coinsEarned = 0, globalReward = null;
  try {
    const result = await submitMellCraft(currentUser.uid, currentUser.username, craftedItem, gameState);
    coinsEarned = result.coinsEarned;
    globalReward = result.globalRewardToGive;

    // Оновлюємо gameState з актуальними даними
    const { getDoc: gd, doc: d } = await import("https://www.gstatic.com/firebasejs/10.7.0/firebase-firestore.js");
    const snap = await gd(d(db, "users", currentUser.uid));
    if (snap.exists()) {
    ...
    showToast("Помилка МеллКрафту: " + e.message, "error");
  }

  await saveData();

  // Якщо є глобальна нагорода — видаємо всім (спрощено: поточному юзеру)
  if (globalReward) {
    _giveMellGlobalReward(globalReward);
  }
  ...
  openModal(`
    <div style="text-align:center;border:2px solid ${r.color};border-radius:12px;padding:24px;background:${r.color}22;">
      ...
      <img src="${craftedItem.img}" style="width:80px;height:80px;object-fit:contain;display:block;margin:12px auto;" onerror="this.src='img/items/m1.png'">
      <div style="font-weight:700;">${craftedItem.name}</div>
      <div style="color:${r.color}">${r.name}</div>
      <div style="color:#888;font-size:12px;">${craftedItem.quality}</div>
      ${craftedItem.premium?'<div style="color:#FFD700;margin-top:4px;">⭐ ПРЕМІУМ</div>':''}
      <div style="margin-top:12px;padding:10px;background:rgba(255,230,102,0.12);border:1px solid #ffe066;border-radius:8px;font-size:14px;">
        🪙 <b style="color:#ffe066;">+${coinsEarned} МеллКоінс</b>
...
function _giveMellGlobalReward(reward) {
  if (!gameState.inventory) gameState.inventory = [];
  if (reward.type === "case" || reward.type === "case+coins") {
    gameState.inventory.push({
      id: generateId(), type: "case",
      caseId: reward.caseId, name: "МеллстройКолекція",
      img: "img/cases/mellstroy.png",
      fromCaseName: "МеллХП Нагорода", source: "mellhp",
      obtainedAt: Date.now(),
    });
    if (reward.coins) {
      gameState.mellCoins = (gameState.mellCoins || 0) + reward.coins;
    }
    showToast("🌍 Глобальна нагорода! " + reward.label, "success");
  } else if (reward.type === "medal") {
    gameState.inventory.push({
      id: generateId(), type: "item",
      itemId: reward.medalId,
      name: "Медаль «МеллстройКрафт 2026»",
      img: "img/items/medal_mellcraft.png",
      rarity: "special", collection: "mellstroy",
      quality: "Прямо з цеху", premium: false,
      fromCase: "mellhp", fromCaseName: "МеллХП Медаль",
      source: "mellhp", obtainedAt: Date.now(),
    });
    showToast("🏅 МЕДАЛЬ РОЗБЛОКОВАНА! МеллстройКрафт 2026!", "success");
  }
  saveData();
}
...
function buildMellWheelSegments() {
  const totalW = MELL_SPIN_TABLE.reduce((s, x) => s + x.weight, 0);
  return MELL_SPIN_TABLE.map((entry, i) => {
    const angle = (entry.weight / totalW) * Math.PI * 2;
    let color = "#2a2a3a";
    if (entry.type === "item") {
    ...
    return { ...entry, angle, color: bright, visual };
  });
}
...
function drawWheel(canvas, segments, rotation) {
  ...
  // Золоті крапки по ободу
  const dotCount = 24;
  for (let i = 0; i < dotCount; i++) {
    const a  = (i / dotCount) * Math.PI * 2 + rotation;
...
function drawArrow(arrowCanvas) {
  ...
  const ax = w / 2;
  // Трикутник вниз (вказує на колесо)
  ctx.beginPath();
  ctx.moveTo(ax, h);
  ctx.lineTo(ax - 11, 0);
  ctx.lineTo(ax + 11, 0);
...
async function _startMellWheelSpin(power) {
  ...
  // Кількість обертів залежить від сили (min 3, max 10)
  const minSpins = 3 + power * 7;
  const totalRot = minSpins * Math.PI * 2;

  // Запускаємо Firestore паралельно
  const spinPromise = doMellSpin(currentUser.uid, gameState).catch(e => { throw e; });

  // Тривалість анімації залежить від сили (min 3s, max 7s)
  ...
  let prizeFromServer = null;
  let animDone = false;

  function animate(now) {
    const canvas = document.getElementById("mell-wheel-canvas");
    if (!canvas) return; // сторінку закрили

    const elapsed = now - startTime;
    const t = Math.min(elapsed / duration, 1);
    const eased = easeOut(t);

    _mellSpring.rotation = startRot + eased * totalRot;
    drawWheel(canvas, segments, _mellSpring.rotation);

    if (t < 1) {
      _mellSpring.animId = requestAnimationFrame(animate);
    } else {
      animDone = true;
      if (prizeFromServer !== null) {
        _snapWheelToPrize(prizeFromServer, segments, canvas, () => _mellFinish(prizeFromServer));
      }
    }
  }
  ...
  // Чекаємо приз з сервера
  try {
    const prize = await spinPromise;
    ...
    prizeFromServer = prize;
    if (animDone) {
      _snapWheelToPrize(prize, segments, document.getElementById("mell-wheel-canvas"), () => _mellFinish(prize));
    }
  } catch (e) {
    cancelAnimationFrame(_mellSpring.animId);
...
// ── ПЛАВНЕ ДОВЕДЕННЯ СТРІЛКИ ДО ВІНПОЗИЦІЇ (фікс телепортації) ────
function _snapWheelToPrize(prize, segments, canvas, onDone) {
  if (!canvas) { onDone(); return; }

  // Знаходимо індекс виграшного сегменту
  ...
  // Середина виграшного сегменту у "абсолютних" координатах
  let segStart = 0;
  for (let i = 0; i < targetIdx; i++) segStart += segments[i].angle;
  const segMid = segStart + segments[targetIdx].angle / 2;

  // Хочемо щоб стрілка (зверху, -π/2) показувала на цей сегмент
  // Тобто: rotation + segMid = -π/2 + 2πk  =>  rotation = -π/2 - segMid + 2πk
  // Обираємо k так щоб додати мінімум кута вперед (не назад)
  const targetAngle = -Math.PI / 2 - segMid;
  const cur  = _mellSpring.rotation % (Math.PI * 2);
  // Нормалізуємо поточне
  let diff = ((targetAngle - cur) % (Math.PI * 2) + Math.PI * 2) % (Math.PI * 2);
  if (diff > Math.PI) diff -= Math.PI * 2; // найкоротший шлях

  // Якщо різниця мала — просто доведемо плавно за 0.5s
  const snapFrom = _mellSpring.rotation;
  const snapTo   = snapFrom + diff;
  const snapDur  = 500;
  const snapStart = performance.now();

  function easeSnap(t) { return t < 0.5 ? 2*t*t : -1+(4-2*t)*t; }

  function animSnap(now) {
    const canvas = document.getElementById("mell-wheel-canvas");
    if (!canvas) { onDone(); return; }
    const t = Math.min((now - snapStart) / snapDur, 1);
    _mellSpring.rotation = snapFrom + easeSnap(t) * (snapTo - snapFrom);
    drawWheel(canvas, segments, _mellSpring.rotation);
    if (t < 1) {
...
async function renderMellCraft() {
  ...
  const contrib = gameState.mellHpContrib || 0;
  const totalHp = _mellGlobalData.totalHp || 0;
  const canSpin = coins >= MELL_SPIN_COST;
  ...
  page.innerHTML = `
    ...
    <!-- Дві колонки: Колесо | Крафт -->
    <div class="mell-two-col">

      <!-- ═══ КОЛЕСО ═══ -->
      <div class="mell-section">
        <div class="mell-section-title">🎰 Колесо фортуни</div>

        <div class="mell-wheel-wrap">
          <canvas id="mell-arrow-canvas" width="30" height="28"></canvas>
          <canvas id="mell-wheel-canvas" width="260" height="260"
            style="max-width:min(260px,100%);height:auto;"></canvas>
        </div>

        <!-- Спідометр сили -->
        <div class="mell-spring-bar-wrap">
          <div class="mell-spring-label">⚡ Сила</div>
        ...
        <!-- Кнопка (альтернатива тягнути) -->
        <button class="mell-spin-action-btn" id="mell-spin-inline-btn"
          onclick="_startMellWheelSpin(0.5)"
          ${!canSpin ? "disabled" : ""}>
          ${canSpin ? "🎰 Крутити (" + MELL_SPIN_COST + " 🪙)" : "❌ Мало МеллКоінс"}
        </button>

        <!-- Таблиця шансів -->
        <details style="margin-top:4px;">
          <summary style="font-size:11px;font-weight:700;color:var(--text-muted);cursor:pointer;padding:2px 0;">
            📊 Шанси
          </summary>
          <div style="background:var(--bg-card2);border:1px solid var(--border);border-radius:8px;padding:8px;margin-top:4px;">
            ${MELL_SPIN_TABLE.map(entry => {
              const totalW = MELL_SPIN_TABLE.reduce((s, x) => s + x.weight, 0);
              const pct    = ((entry.weight / totalW) * 100).toFixed(1);
              return `<div style="display:flex;justify-content:space-between;font-size:11px;padding:3px 0;border-bottom:1px solid var(--border);">
                ...
                <span style="font-weight:700;color:#ffe066;">${pct}%</span>
              </div>`;
            }).join("")}
          </div>
        </details>
      ...
      <div class="mell-section">
        ...
        <p style="color:var(--text-muted);font-size:12px;line-height:1.4;">
          5 предметів <b>Меллстрой</b> однієї рідкості.<br>
          За крафт: <b style="color:#ffe066;">МеллКоінс</b> + <b style="color:#5ddb8a;">+${MELL_CRAFT_HP} МеллХП</b>
        </p>

        <div id="mellcraft-slots" class="craft-slots"></div>
        <div id="mellcraft-info" style="margin:4px 0;"></div>
        <button id="mellcraft-btn" class="btn-primary btn-full" onclick="doMellCraftAction()" disabled>
          ⚗️ Крафтити
        </button>

        <div style="font-size:12px;color:var(--text-muted);margin-top:10px;font-weight:700;">
          🎒 Предмети Меллстрой:
        </div>
        <div id="mellcraft-picker" class="craft-picker"></div>
      </div>
  ...
  const segments = buildMellWheelSegments();
  _mellSpring.segments = segments;
  _mellSpring.spinning = false;
  _mellSpring.power    = 0;

  // Завантажуємо картинки і малюємо
  ...
  if (wheelCanvas && arrowCanvas) {
    drawArrow(arrowCanvas);
...
// ── _buildMellHPBarHTML (без змін, але має бути доступна) ──────────
function _buildMellHPBarHTML(totalHp) {
  const maxHp = 1000;
  const pct   = Math.min(100, Math.round((totalHp / maxHp) * 100));
  const claimed = _mellGlobalData.claimedLevels || [];
  ...
  return `
    ...
    <div style="font-size:11px;color:var(--text-muted);margin-top:6px;text-align:center;">
      ${totalHp >= maxHp
        ? "🏅 Глобальний шлях завершено!"
        : "Наступна: " + (MELL_HP_REWARDS.find(m => m.hp > totalHp)?.reward.label || "—") +
          " при " + (MELL_HP_REWARDS.find(m => m.hp > totalHp)?.hp || "?") + " ХП"}
    </div>
  `;
}
...
// ── ЗАСТАРІЛІ ФУНКЦІЇ (залишаємо для window.* exports але вони не використовуються) ──
function openMellSpinModal()  { /* перенесено в inline */ }
async function doMellSpinAction() { /* перенесено в _startMellWheelSpin */ }
function injectMellSpinStyles()  { /* стилі тепер в injectMellCraftStyles */ }
function _finishSpin(prize, segments, resultEl, btnsEl, againBtn, wheelCanvas) {
  // Сумісність — делегуємо
  _mellFinish(prize);
}
...
window.doMellSpinAction        = doMellSpinAction;
...
window._finishSpin             = _finishSpin;
