...
function drawWheel(canvas, segments, rotation) {
  ...
  ctx.translate(cx, cy);
  ctx.rotate(rotation);
  ctx.drawImage(_mellOffscreenCanvas, -cx, -cy);
  ctx.restore();
}
...
async function _startMellWheelSpin(power) {
  ...
  const minRounds = 5 * Math.PI * 2;
  const totalDuration = 3500 + power * 2000;

  function easeOut(t) { return 1 - Math.pow(1 - t, 4); }

  // Запускаємо Firestore ОДРАЗУ паралельно
  const prizePromise = doMellSpin(currentUser.uid, gameState, currentUser.username || "")
    .catch(e => {
      console.error("doMellSpin error:", e);
      showToast("❌ " + e.message, "error");
      return null;
    });

  // Стан для анімації
  let prizeResolved = null;
  let finalDiff = null;
  let snapStartTime = null;
  let snapStartRot = null;
  const SNAP_DURATION = 1200;

  // Коли приходить приз — обчислюємо фінальний кут
  prizePromise.then(prize => {
    prizeResolved = prize;
    if (!prize) return;
    ...
    const curRot = _mellSpring.rotation;
    const curNorm = ((curRot % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2);
    const tgtNorm = ((targetAngle % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2);
    let diff = tgtNorm - curNorm;
    if (diff < 0) diff += Math.PI * 2;
    // Мінімум пів оберту щоб не виглядало різко
  ...
  // Єдиний анімаційний цикл
  await new Promise(resolve => {
    function animate(now) {
      const c = document.getElementById("mell-wheel-canvas");
      if (!c) { resolve(); return; }

      // Фаза 2: snap до вінпосу
      if (finalDiff !== null && snapStartTime !== null) {
        const elapsed = now - snapStartTime;
        const t = Math.min(elapsed / SNAP_DURATION, 1);
        _mellSpring.rotation = snapStartRot + easeOut(t) * finalDiff;
      ...
      // Фаза 1: вільне обертання
      const elapsed = now - startTime;
      const t = Math.min(elapsed / totalDuration, 1);
      // Якщо приз прийшов і вийшов мінімум 1 оберт — починаємо snap
      if (prizeResolved !== null && elapsed > 1000) {
        // Snap стартує наступного кадру (finalDiff вже встановлено в .then)
        if (finalDiff !== null) {
          const snapT = Math.min((now - snapStartTime) / SNAP_DURATION, 1);
          _mellSpring.rotation = snapStartRot + easeOut(snapT) * finalDiff;
          drawWheel(c, segments, _mellSpring.rotation);
          if (snapT >= 1) { resolve(); return; }
          _mellSpring.animId = requestAnimationFrame(animate);
          return;
        }
      }

      // Продовжуємо вільне обертання
      _mellSpring.rotation = startRot + easeOut(t) * (minRounds + Math.PI * 2 * 2);
      drawWheel(c, segments, _mellSpring.rotation);

      if (t >= 1 && prizeResolved === null) {
        // Анімація закінчилась але приз ще не прийшов — чекаємо
        _mellSpring.animId = requestAnimationFrame(animate);
        return;
      }

      if (t < 1 || finalDiff === null) {
        _mellSpring.animId = requestAnimationFrame(animate);
      } else {
        resolve();
      }
    ...
    <div class="mell-two-col">
      <div class="mell-section">
        ...
        <div class="mell-wheel-wrap">
          ...
          <canvas id="mell-wheel-canvas" width="260" height="260"></canvas>
        </div>
