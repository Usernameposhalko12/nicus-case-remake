...
gameState = new Proxy(gameState, {
    set(target, property, value) {
        if (property === 'balance') {
            const oldBalance = target[property] || 0;
            const diff = value - oldBalance;

            if (diff > 0) {
                // Викликаємо хук нарахування для БП
                if (typeof bpHookEarn === 'function') {
                    bpHookEarn(diff); 
                }
                // Якщо у тебе завдання саме на MellCoins:
                if (typeof bpHookMellCoins === 'function') {
                    bpHookMellCoins(diff);
                }
            } 
            
            // Також можна автоматизувати витрати (завдання "Витрать 5000")
            if (diff < 0) {
                if (typeof bpHookSpend === 'function') {
                    bpHookSpend(Math.abs(diff));
                }
            }
        }
