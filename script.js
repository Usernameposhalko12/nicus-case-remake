...
async function openMellSpinModal() {
  ...
  openModal(`
    ...
    <div style="background:var(--bg-card2);border:1px solid var(--border);border-radius:10px;padding:12px;margin-bottom:14px;">
      <div style="font-size:11px;font-weight:700;color:var(--text-muted);text-transform:uppercase;margin-bottom:8px;">Таблиця шансів</div>
      ${tableHtml}
    </div>
    <div style="display:flex;gap:8px;">
      <button class="btn-primary" style="flex:1;" onclick="doMellSpinAction()"
...
async function doMellSpinAction() {
  const btn = document.querySelector('#modal-body .btn-primary');
  if (btn) { btn.disabled = true; btn.textContent = "⏳ Крутимо..."; }

  try {
    const prize = await doMellSpin(currentUser.uid, gameState);
    ...
    const r = prize.rarity ? RARITIES[prize.rarity] : null;
    const color = r ? r.color : "#5ddb8a";

    openModal(`
      <div style="text-align:center;border:2px solid ${color};border-radius:12px;padding:24px;background:${color}18;">
        <div style="font-size:36px;margin-bottom:8px;">🎰</div>
        <div style="font-size:17px;font-weight:700;margin-bottom:14px;">Ти виграв!</div>
        <div style="font-size:16px;font-weight:700;color:${color};">${prize.label}</div>
        <div style="color:var(--text-muted);font-size:13px;margin-top:8px;">Залишилось 🪙: <b>${gameState.mellCoins}</b></div>
        <div style="display:flex;gap:8px;margin-top:16px;">
          <button class="btn-primary" style="flex:1;" onclick="openMellSpinModal()"
            ${gameState.mellCoins >= MELL_SPIN_COST ? "" : "disabled"}>
            🎰 Ще раз
          </button>
    ...
    showToast("Помилка: " + e.message, "error");
    if (btn) { btn.disabled = false; btn.textContent = `🎰 Крутити (${MELL_SPIN_COST} 🪙)`; }
  }
}
