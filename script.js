...
async function openMellSpinModal() {
  ...
  openModal(`
    ...
    <details style="margin-bottom:12px;">
      <summary style="font-size:12px;font-weight:700;color:var(--text-muted);cursor:pointer;padding:6px 0;">📊 Таблиця шансів</summary>
      <div style="background:var(--bg-card2);border:1px solid var(--border);border-radius:10px;padding:10px;margin-top:6px;">
        ${tableHtml}
      </div>
    </details>
    <div id="mell-spin-roulette" style="display:none;margin-bottom:12px;">
      <div class="roulette-wrap">
        <div class="roulette-arrow">▼</div>
        <div class="roulette-viewport">
          <div class="roulette-strip" id="mell-roulette-strip"></div>
        </div>
      </div>
    </div>
    <div id="mell-spin-result" style="display:none;text-align:center;margin-bottom:14px;"></div>
    <div id="mell-spin-btns" style="display:flex;gap:8px;">
      <button class="btn-primary" style="flex:1;" id="mell-spin-go-btn" onclick="doMellSpinAction()"
        ${coins < MELL_SPIN_COST ? "disabled" : ""}>
        🎰 Крутити (${MELL_SPIN_COST} 🪙)
      </button>
      <button class="btn-secondary" style="flex:1;" id="mell-spin-skip-btn" style="display:none;" onclick="skipMellSpin()">⚡ Пропустити</button>
      <button class="btn-secondary" style="flex:1;" onclick="closeModal()">Закрити</button>
    </div>
    <button id="mell-spin-again-btn" class="btn-primary btn-full" style="display:none;margin-top:8px;" onclick="openMellSpinModal()">🎰 Ще раз</button>
  `, true);
}

function skipMellSpin() {
  const stripEl = document.getElementById("mell-roulette-strip");
  if (stripEl) {
    stripEl.style.transition = "none";
    stripEl.dispatchEvent(new Event("transitionend"));
  }
...
async function doMellSpinAction() {
  ...
  const skipBtn = document.getElementById("mell-spin-skip-btn");
  if (goBtn) { goBtn.disabled = true; goBtn.style.display = "none"; }
  if (skipBtn) { skipBtn.style.display = "flex"; }

  try {
    // Спочатку визначаємо приз локально (для анімації), потім Firestore
    const totalW = MELL_SPIN_TABLE.reduce((s, x) => s + x.weight, 0);
    let r = Math.random() * totalW;
    let prizeEntry = MELL_SPIN_TABLE[MELL_SPIN_TABLE.length - 1];
    for (const entry of MELL_SPIN_TABLE) {
      r -= entry.weight;
      if (r <= 0) { prizeEntry = entry; break; }
    }

    const ITEM_W  = 110, TOTAL_W = 120, COUNT = 60;
    const winPos  = 46 + Math.floor(Math.random() * 4);
    const strip   = buildMellSpinStrip(winPos, prizeEntry);
    const stripHtml = strip.map(e => mellSpinCardHtml(e, ITEM_W)).join("");

    // Показуємо рулетку
    const roulDiv = document.getElementById("mell-spin-roulette");
    if (roulDiv) roulDiv.style.display = "block";

    const stripEl = document.getElementById("mell-roulette-strip");
    if (stripEl) {
      stripEl.innerHTML = stripHtml;
      const vpWidth = stripEl.parentElement.offsetWidth;
      const targetX = winPos * TOTAL_W + ITEM_W / 2 - vpWidth / 2 + (Math.random() - 0.5) * ITEM_W * 0.5;
      stripEl.style.transition = "none";
      stripEl.style.transform  = "translateX(0)";

      // Паралельно запускаємо Firestore транзакцію
      const spinPromise = doMellSpin(currentUser.uid, gameState);

      requestAnimationFrame(() => requestAnimationFrame(() => {
        stripEl.style.transition = "transform 4.5s cubic-bezier(0.15, 0.8, 0.35, 1)";
        stripEl.style.transform  = `translateX(-${targetX}px)`;
      }));

      stripEl.addEventListener("transitionend", async () => {
        if (skipBtn) skipBtn.style.display = "none";
        // Чекаємо завершення Firestore
        let prize;
        try {
          prize = await spinPromise;
        } catch(e) {
          showToast("Помилка: " + e.message, "error");
          if (goBtn) { goBtn.disabled = false; goBtn.style.display = "flex"; }
          return;
        }
        ...
        let color = "#5ddb8a";
        if (prize.type === "item") {
          const rr = RARITIES[prize.rarity];
          color = rr ? rr.color : "#888";
        } else {
          color = "#ffe066";
        }
        ...
        if (resultEl) {
          resultEl.style.display = "block";
          resultEl.innerHTML = `
            <div style="border:2px solid ${color};border-radius:14px;padding:18px;background:${color}18;display:inline-block;min-width:160px;">
              <div style="color:#888;font-size:13px;margin-bottom:6px;">🎉 Ти виграв!</div>
              <div style="font-size:15px;font-weight:700;color:${color};">${prize.label}</div>
              <div style="color:var(--text-muted);font-size:12px;margin-top:8px;">🪙 Залишилось: <b>${gameState.mellCoins}</b></div>
            </div>`;
        }
        ...
        if (againBtn) {
          ...
          againBtn.disabled = gameState.mellCoins < MELL_SPIN_COST;
        }

        // Оновлюємо МеллКрафт сторінку якщо відкрита
