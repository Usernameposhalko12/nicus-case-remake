...
async function _startMellWheelSpin(power) {
  ...
  // Запускаємо Firestore ОДРАЗУ — паралельно з анімацією
  ...
  // Мінімум обертів до зупинки (5 повних + частина до вінпосу)
  const minRounds = 5 * Math.PI * 2;
  const totalDuration = 3500 + power * 2000;
  const startTime = performance.now();
  const startRot = _mellSpring.rotation;

  // Чекаємо приз — але не більше ніж totalDuration - 800ms (щоб залишився час на easeOut)
  const prize = await Promise.race([
    prizePromise,
    new Promise(resolve => setTimeout(() => resolve("timeout"), totalDuration - 800))
  ]);

  if (prize === "timeout") {
    // Сервер не відповів вчасно — чекаємо далі вже зупинившись
    _mellSpring.spinning = false;
    if (canvas) canvas.classList.remove("spinning");
    _setSpinBtnDisabled(false);
    showToast("❌ Сервер не відповів вчасно", "error");
    // Синхронізуємо монети
    try {
      const snap = await getDoc(doc(db, "users", currentUser.uid));
      if (snap.exists()) {
  ...
  // Тепер знаємо вінпос — будуємо фінальний кут
  ...
  // Скільки треба крутити від startRot щоб попасти в targetAngle + мінімум 5 обертів
  const tgtNorm = ((targetAngle % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2);
  const startNorm = ((startRot % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2);
  let diff = tgtNorm - startNorm;
  if (diff < 0) diff += Math.PI * 2;
  // Додаємо мінімум 5 повних обертів
  diff += minRounds;

  // Одна плавна анімація від початку до вінпосу
  await new Promise(resolve => {
    function animate(now) {
      ...
      _mellSpring.rotation = startRot + easeOut(t) * diff;
