...
import {
  ...
  getAllUsernames, onAuthStateChanged, db // Додали db
} from "./firebase.js";

// ДОДАЙ ЦЕЙ ІМПОРТ НИЖЧЕ (це важливо для роботи бази)
import { 
...
window.addEventListener('DOMContentLoaded', () => {
  onAuthStateChanged(auth, (user) => {
    if (user) {
      // Підписуємося на "живі" оновлення профілю (баланс, предмети)
      onSnapshot(doc(db, "users", user.uid), (snapshot) => {
        if (snapshot.exists()) {
          ...
          // Показуємо меню і йдемо в гру
          document.getElementById('navbar').style.display = 'flex';
          document.getElementById('bottom-nav').style.display = 'flex';
          updateUI();
          
          // Якщо ми були на сторінці логіну — перекидаємо на головну
      ...
      // Якщо не залогінені — показуємо форму входу
      currentUser = null;
