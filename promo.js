...
async function activatePromoCode(code, currentUser, gameState, deps) {
  ...
  return await runTransaction(db, async (tx) => {
    ...
    // Записуємо в історію гравця (окремий doc)
    ...
    return `
      <div class="promo-admin-card">
        ...
        <div class="promo-admin-actions">
          ...
          <button class="btn-sm btn-cancel" onclick="window._adminDeletePromo('${p.id}')">🗑</button>
        </div>
      </div>`;
  }).join("");
  ...
  window._adminViewPromoUsers = async (code) => {
    ...
    deps.openModal(`
      ...
      <div style="max-height:50vh;overflow-y:auto;">
        ${usedBy.length
          ? usedBy.map(uid => `<div style="padding:8px 12px;background:var(--bg-card2);border-radius:8px;margin-bottom:6px;font-size:13px;font-family:'Courier New',monospace;">${uid}</div>`).join("")
          : '<div style="color:var(--text-muted);text-align:center;padding:20px;">Ніхто ще не використовував</div>'}
      </div>
      <button class="btn-secondary btn-full" style="margin-top:12px;" onclick="window._adminCreatePromoModal()">← Назад</button>
    `);
  };
...
function adminCreatePromoModal(deps) {
  ...
  deps.openModal(`
    ...
    <div class="promo-create-form" style="max-height:70vh;overflow-y:auto;">
      ...
      <div style="display:flex;gap:8px;margin-top:4px;">
        ...
        <button class="btn-secondary" style="flex:1;" onclick="window._adminPromoBack()">← Назад</button>
      </div>
    </div>
  `, true);
  ...
  window._adminPromoBack = () => renderAdminPromoPanel(deps);
  ...
  window._adminSavePromo = async () => {
    ...
    const rewardType = activeTab?.getAttribute("data-type") || "balance";
    // Зберігаємо тип у data-атрибуті
    const tabEl = document.querySelector(".promo-reward-tab.active");
    const ttype = tabEl?.onclick?.toString().match(/'([^']+)'/)?.[1] || "balance";
...
function buildRewardFromForm() {
  ...
  if (text.includes("Предмет")) {
    ...
    const itemEl = document.getElementById("promo-item-id");
