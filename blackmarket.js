...
function injectBlackMarketStyles() {
  ...
  s.textContent = `
    ...
    .bm-pin-aimed .bm-pin-cyl {
      transform: translateY(-4px);
      box-shadow: 0 0 8px rgba(224,173,92,.7);
      border-color: #c97a3d;
    }
    ...
    .bm-rail {
      position: absolute;
      left: 12px; right: 66px; bottom: 6px;
      height: 6px;
      background: #101113;
      border-radius: 3px;
    ...
    .bm-tool-arm {
      width: 8px;
      height: 78%;
      background: linear-gradient(90deg, #d9dde2, #7d8288 45%, #d9dde2 55%, #7d8288);
      border-radius: 3px;
...
function openLockpickMinigame(currentUser, gameState, target, lockpick) {
  ...
  const paint = () => {
    const html = `
      ...
      <div class="bm-lock-wrap">
        <div class="bm-lockpick-progress-label">Підведи відмичку до пружини і натисни — вгадай ${solvedCount + 1}-шу з ${SPRING_COUNT}</div>
        ...
        <div class="bm-housing">
          ...
          <div class="bm-pins">
            ${Array.from({ length: SPRING_COUNT }).map((_, i) => {
              ...
              return `<button class="bm-pin${locked ? " bm-pin-locked" : ""}${active ? " bm-pin-active" : ""}" data-idx="${i}">
          ...
          <div class="bm-tool"><div class="bm-tool-arm"></div><div class="bm-tool-hook"></div></div>
          <div class="bm-rail">
        ...
        <div class="bm-pick-controls">
          <button class="bm-ctrl-btn" id="bm-move-left">◀</button>
          <button class="bm-press-btn" id="bm-press">🔨 Натиснути</button>
          <button class="bm-ctrl-btn" id="bm-move-right">▶</button>
        </div>
        <button class="btn-secondary btn-full" id="bm-abort-lockpick" style="margin-top:10px;">Скасувати спробу</button>
      </div>
    `;
    _bmDeps.openModal(html, false);

    document.querySelectorAll(".bm-pin").forEach(btn => {
  ...
  const handlePress = () => {
    ...
    if (springNum === secret[solvedCount]) {
      ...
      solvedCount++;
      if (solvedCount >= SPRING_COUNT) { paint(); setTimeout(() => finish(true), 250); return; }
