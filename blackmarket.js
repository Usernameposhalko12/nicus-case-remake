...
function openLockpickMinigame(currentUser, gameState, target, lockpick) {
  ...
  const paint = () => {
    const html = `
      ...
      <div class="bm-lock-wrap">
        <div class="bm-lockpick-progress-label">Вгадай, яку пружину натиснути ${solvedCount + 1}-ю з ${SPRING_COUNT}</div>
        ...
        <div class="bm-housing">
          ...
          <div class="bm-pins">
            ${Array.from({ length: SPRING_COUNT }).map((_, i) => {
              ...
              return `<button class="bm-pin${locked ? " bm-pin-locked" : ""}" data-spring="${springNum}" ${locked ? "disabled" : ""}>
    ...
    document.querySelectorAll(".bm-pin:not(:disabled)").forEach(btn => {
      btn.onclick = () => handleGuess(parseInt(btn.getAttribute("data-spring"), 10), btn);
    });
  ...
  const handleGuess = (springNum, btnEl) => {
    if (springNum === secret[solvedCount]) {
      solvedFlags[springNum - 1] = true;
      solvedCount++;
      if (solvedCount >= SPRING_COUNT) { setTimeout(() => finish(true), 220); paint(); return; }
      paint();
    } else {
      mistakes++;
      if (btnEl) {
        btnEl.parentElement && btnEl.classList.add("bm-pin-wrong");
      }
      if (mistakes >= MAX_MISTAKES) { setTimeout(() => finish(false), 250); return; }
