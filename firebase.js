...
import {
  ...
  orderBy, sendClanMessage, subscribeClanChat,
onSnapshot, serverTimestamp, where, runTransaction
...
async function removeMarketListing(docId, sellerUid) {
  await runTransaction(db, async (tx) => {
    ...
    inv.push({ ...listing.item, id: listing.item?.id || ('ret_' + Date.now().toString(36)), obtainedAt: Date.now() });
