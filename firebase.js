...
// ============================================
// Ставка знімається одразу при старті раунду (cupsPlaceBet) — так гравець не
// може почати гру, не маючи 50 нікусів. Виграш/бонус досвіду нараховується
// окремою транзакцією після завершення раунду (cupsSettleGame). Хто саме
// виграв визначається на клієнті (той самий рівень довіри, що й у крафті —
// 50/50 теж рахується в script.js), тут лише атомарно рухаємо баланс/xp.

const CUPS_GAME_COST     = 50;
const CUPS_LEVEL_REWARDS = { 1: 100, 2: 150, 3: 200, 4: 250 };

async function cupsPlaceBet(uid) {
  let newBalance = 0;
  await runTransaction(db, async (tx) => {
    ...
    if (!snap.exists()) throw new Error("Профіль не знайдено!");
    const bal = snap.data().balance || 0;
    if (bal < CUPS_GAME_COST) throw new Error("Недостатньо нікусів! Потрібно " + CUPS_GAME_COST + ".");
    newBalance = bal - CUPS_GAME_COST;
    tx.update(ref, { balance: newBalance });
  });
  return newBalance;
}

async function cupsSettleGame(uid, win, level) {
  const reward = win ? (CUPS_LEVEL_REWARDS[level] || 0) : 0;
  const xpGain = win ? 15 : 3; // трохи досвіду навіть за програш, щоб раунд не був геть марним
  let result = { balance: 0, xp: 0, level: 1, leveledUp: false };
  await runTransaction(db, async (tx) => {
    const ref  = doc(db, "users", uid);
    const snap = await tx.get(ref);
    if (!snap.exists()) throw new Error("Профіль не знайдено!");
    const data = snap.data();
    let bal  = (data.balance || 0) + reward;
    let xp   = (data.xp || 0) + xpGain;
    let lvl  = data.level || 1;
    let leveledUp = false;
    let need = lvl * 100;
    while (xp >= need) { xp -= need; lvl += 1; need = lvl * 100; leveledUp = true; }
    tx.update(ref, { balance: bal, xp, level: lvl });
    result = { balance: bal, xp, level: lvl, leveledUp };
  });
  return { ...result, reward };
}

// ============================================
// МІНІ-ГРА: 💣 СЛІПИЙ САПЕР
// ============================================
// Той самий сапер, тільки без підказок (жодних цифр біля відкритих клітинок
// — тільки "безпечно" чи "міна"). Одна спроба коштує 10 нікусів, кожна
// успішно відкрита клітинка = 2 нікуси. Балів немає доки не станеться
// "смерть" (клік на міну) або поле не буде очищене повністю — саме тоді всі
// відкриті клітинки конвертуються у нікуси однією транзакцією. Явної кнопки
// "забрати достроково" немає — це навмисно, гра триває до підриву.

const BLIND_MINE_COST      = 10;
const BLIND_MINE_REWARD_PER_CELL = 2;

async function blindMinePlaceBet(uid) {
  let newBalance = 0;
  await runTransaction(db, async (tx) => {
    const ref  = doc(db, "users", uid);
    const snap = await tx.get(ref);
    if (!snap.exists()) throw new Error("Профіль не знайдено!");
    const bal = snap.data().balance || 0;
    if (bal < BLIND_MINE_COST) throw new Error("Недостатньо нікусів! Потрібно " + BLIND_MINE_COST + ".");
    newBalance = bal - BLIND_MINE_COST;
    tx.update(ref, { balance: newBalance });
  });
  return newBalance;
}

async function blindMineSettle(uid, openedCount) {
  const reward = openedCount * BLIND_MINE_REWARD_PER_CELL;
  const xpGain = Math.min(40, 2 + openedCount); // трохи досвіду пропорційно ризику
  let result = { balance: 0, xp: 0, level: 1, leveledUp: false };
  await runTransaction(db, async (tx) => {
    const ref  = doc(db, "users", uid);
    const snap = await tx.get(ref);
...
export {
  ...
  // Стаканчик
  CUPS_GAME_COST, CUPS_LEVEL_REWARDS, cupsPlaceBet, cupsSettleGame,
  // Сліпий сапер
  BLIND_MINE_COST, BLIND_MINE_REWARD_PER_CELL, blindMinePlaceBet, blindMineSettle,
};