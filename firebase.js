...
// ДЮП-ФІКС: транзакція з перечитуванням лота
async function buyMarketItem(buyerUid, buyerGameState, listing) {
  ...
  await runTransaction(db, async (tx) => {
    ...
    if (!listingSnap.exists()) throw new Error("Лот вже знятий з продажу!");

    const price = listingSnap.data().price;
    const item  = listingSnap.data().item;
    ...
    const newItem  = { ...item, id: item?.id || ('m_' + Date.now().toString(36)), obtainedAt: Date.now() };
...
async function clanVaultWithdraw(clanId, withdrawerUid, type, amount, itemId) {
  await runTransaction(db, async (tx) => {
    ...
    if (type === "balance") {
      ...
      // Витягуємо предмет з масиву общака
      const taken = vault.inventory.splice(idx, 1)[0];

      // Очищуємо предмет від метаданих пожертви, щоб не було помилок з undefined
      const cleanedItem = { ...taken, obtainedAt: Date.now() };
    ...
    // Оновлюємо дані клану після змін в общаку
    tx.update(clanRef, { vault });
  });
}
...
[span_11](start_span)async function getUserProfile(username) {[span_11](end_span)
// ...
...
async function clanVaultWithdraw(clanId, withdrawerUid, type, amount, itemId) {
  await runTransaction(db, async (tx) => {
    ...
    if (type === "balance") {
      ...
      if (idx === -1) throw new Error("Предмет не знайдено в общаку!");
      const taken = vault.inventory.splice(idx, 1)[0];
      const uInv  = [...(user.inventory || []), { ...taken, donatedBy: undefined, donatedAt: undefined, obtainedAt: Date.now() }];
      tx.update(userRef, { inventory: uInv });
    }
    tx.update(clanRef, { vault });
  });
