...
// але взаємодія з ними обмежена: не можна додати в друзі, обміняти чи подарувати
// предмет (це фільтрується на рівні UI в script.js). Кожен бот щомісяця має
// бюджет 10000 нікусів, купує лоти на ринку мінімум на 25% нижче середньої
// ринкової ціни цього предмета, знищуючи куплений предмет (він нікуди не
// додається — просто видаляється з ринку). Раз на 30 хв кожен бот здійснює
// щонайбільше 1 покупку. Кожну неділю боти додатково проводять "зачистку
// ринку" — скуповують усі лоти, що залишились, у межах свого бюджету.
...
// Атомарна купівля лоту ботом: продавцю зараховується оплата (мінус 10% комісії
// ринку), предмет знищується (просто видаляється з ринку, нікуди не додається).
async function _buyListingAsBot(bot, listing) {
  ...
  await runTransaction(db, async (tx) => {
    ...
    tx.update(sellerRef, { balance: sellerBal + payout });
    tx.delete(listingRef); // предмет знищується ботом

    notifyPayload = {
      sellerId: liveData.sellerId,
      buyerName: bot.username,
      itemName: liveData.item?.name || "Предмет",
  ...
  if (notifyPayload) {
    addNotification(notifyPayload.sellerId, {
      ...
      message: notifyPayload.buyerName + " (бот) купив(ла) у тебе \"" + notifyPayload.itemName + "\" за " +
        notifyPayload.price + " нікусів (комісія ринку 10%, зараховано " + notifyPayload.payout + "). Предмет знищено.",
    }).catch(() => {});
  }
}

// Звичайний цикл: кожен бот здійснює максимум 1 покупку за тік —
// лот, ціна якого мінімум на 25% нижче середньої ринкової ціни цього предмета.
async function _botsBuyCycle() {
  ...
  listings.forEach(l => {
    const n = l.item?.name || "?";
    (groups[n] = groups[n] || []).push(l.price);
  });
  const avgByName = {};
  Object.keys(groups).forEach(n => {
    avgByName[n] = groups[n].reduce((a, b) => a + b, 0) / groups[n].length;
  });
  ...
  for (const name of BOT_USERNAMES) {
    try {
      ...
      const candidates = fresh
        ...
        .filter(l => {
          const avg = avgByName[l.item?.name || "?"] ?? l.price;
          return l.price <= avg * (1 - BOT_DISCOUNT_RATIO) && l.price <= bot.balance;
        })
