...
async function _buyListingAsBot(bot, listing) {
  ...
  await runTransaction(db, async (tx) => {
    ...
    tx.update(sellerRef, { balance: sellerBal + payout });
    tx.update(botRef,    { inventory: botInv, balance: botBal - liveData.price, lastSeen: Date.now() });
    tx.delete(listingRef);
...
async function _botsBuyCycle(disabledBots = []) {
  for (const name of BOT_USERNAMES) {
    ...
    try {
      ...
      await _buyListingAsBot(bot, target); // баланс і інвентар оновлюються всередині транзакції
    } catch (e) {
