...
export async function doMellSpin(uid, gameState, username) {
  ...
  // Тільки списуємо монети — інвентар додається окремо в script.js
  await runTransaction(db, async (tx) => {
    const userRef  = doc(db, "users", uid);
    const userSnap = await tx.get(userRef);
    if (!userSnap.exists()) throw new Error("Профіль не знайдено!");
  ...
  // Будуємо приз і одразу додаємо в інвентар окремим updateDoc
  const prizeItem = buildPrizeItem(prize, username);
  const snap = await getDoc(doc(db, "users", uid));
  if (snap.exists()) {
    const inv = [...(snap.data().inventory || []), prizeItem];
    await updateDoc(doc(db, "users", uid), { inventory: inv });
  }

  return { ...prize, _prizeItem: prizeItem };
}
