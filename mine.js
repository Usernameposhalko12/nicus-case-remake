...
// ОПТИМ: менше читань/записів Firestore без візуальних змін
// ============================================
...
// ══════════════════════════════════════════
// ОПТИМ: відстеження "брудного" стану для savePlayer
// ══════════════════════════════════════════
let _playerDirty    = false; // позиція/hp змінились
let _inventoryDirty = false; // інвентар змінився

// ══════════════════════════════════════════
// ОПТИМ: батчинг модів тайлів — збираємо всі зміни за 300мс, пишемо одним запитом
// ══════════════════════════════════════════
let _pendingMods = {}; // { "tx,ty": id } — накопичені зміни ще не відправлені

// ══════════════════════════════════════════
// ОПТИМ: кешування стану позиції для pushPos — не пушити якщо не рухався
// ══════════════════════════════════════════
let _lastPushedPos = { x: -9999, y: -9999, dir: 1, mining: false };
const POS_PUSH_THRESHOLD = 4; // пікселів — мінімальний рух для пушу

// ══════════════════════════════════════════
// ОПТИМ: HUD оновлення — тільки при зміні, не щокадру
// ══════════════════════════════════════════
...
export async function initMinePage() {
 ...
 if (worldSnap.exists()) {
  ...
  worldSeed   = data.seed || Date.now(); // ← фікс
  _worldMod   = data.mods || {};
  
  // Якщо seed не було — записуємо
  if (!data.seed) {
    await updateDoc(worldRef, { seed: worldSeed });
  }
} else {
  worldSeed = Date.now();
  ...
  // ОПТИМ: збільшено інтервал збереження з 8с до 15с — рідше пишемо в Firestore.
  // Додатково savePlayer пропустить запис якщо нічого не змінилось (_playerDirty).
  _saveTimer = setInterval(savePlayer, 15000);
...
// ОПТИМ: батчинг записів тайлів
// Замість 1 запиту на кожен зламаний/поставлений тайл,
// накопичуємо зміни в _pendingMods і пишемо всі одним updateDoc через 400мс.
// При швидкому копанні (наприклад 5 тайлів за 300мс) — все одно 1 запит.
function pushTileMod(tx, ty, id) {
  const key = tx + "," + ty;
  _worldMod[key]     = id;
  _pendingMods[key]  = id; // додаємо до батчу

  clearTimeout(_pushModTimer);
  _pushModTimer = setTimeout(async () => {
    if (!Object.keys(_pendingMods).length) return;
const batch = { ..._pendingMods };
    _pendingMods = {}; // очищаємо до відправки, щоб нові не загубились

    // Будуємо об'єкт { "mods.tx,ty": id, ... } для одного updateDoc
    const update = {};
  ...
  }, 400); // 400мс — достатньо для батчингу серії копань
}

// ОПТИМ: зберігаємо тільки якщо щось змінилось (_playerDirty або _inventoryDirty)
async function savePlayer() {
  const uid = auth.currentUser?.uid;
  if (!uid || !_player) return;
  if (!_playerDirty && !_inventoryDirty) return; // нічого не змінилось — пропускаємо
...
// ОПТИМ: pushPos пропускає запит якщо позиція/стан не змінились суттєво.
// Поріг: 4px руху АБО зміна dir/mining — тоді пушимо.
async function pushPos() {
  ...
  if (!moved && !stateChanged) return; // стоїть нерухомо — не пишемо
...
function gameLoop(ts) {
  ...
  // ОПТИМ: pushPos тепер має внутрішній threshold — можна залишити 100мс,
  // але реально пишемо в Firestore лише якщо гравець рухається.
  _posPushMs += rawDt;
  if (_posPushMs > 100) { _posPushMs = 0; pushPos(); }
}
...
function update(dt) {
  ...
  // ОПТИМ: позначаємо dirty тільки якщо справді змінилась позиція або hp
...
function doBuild() {
  ...
  _inventoryDirty = true; // ОПТИМ: позначаємо зміну інвентаря
  updateHotbarUI();
  _mineActive = false;
  _keys["Mouse0"] = false;
}
...
function render() {
  ...
  // ОПТИМ: HUD оновлюємо тільки якщо є зміни, не щокадру
...
function updateHotbarUI() {
  ...
  _hudHotbarDirty = false; // скидаємо флаг після рендеру
}
...
const _onKeyDown = (e) => {
  ...
  if (e.key >= "1" && e.key <= "8") {
    _hotbarSel = +e.key - 1;
    _hudHotbarDirty = true; // ОПТИМ: через dirty flag
  }
...
window.mineHotbarSel = (i) => {
  _hotbarSel = i;
  _hudHotbarDirty = true; // ОПТИМ: через dirty flag замість прямого виклику
};
