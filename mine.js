...
// ОПТИМ: ~43k читань + ~14k записів на 4 гравці/1год
// ============================================
...
// ── ОПТИМ: батчинг модів тайлів ─────────────
let _pendingMods = {};

// ── ОПТИМ: кеш позиції — не пушити якщо не рухався ─
// Збільшено поріг до 8px (було 4px) → вдвічі менше записів
let _lastPushedPos = { x: -9999, y: -9999, dir: 1, mining: false };
const POS_PUSH_THRESHOLD = 8;

// ── ОПТИМ: HUD dirty flags ───────────────────
...
export async function initMinePage() {
  ...
  // ОПТИМ: generateWorld в setTimeout щоб не блокувати потік
  ...
  // ОПТИМ: збереження кожні 30с замість 15с
  // + savePlayer пропускає запис якщо нічого не змінилось
...
// ОПТИМ: батчинг записів тайлів — один updateDoc на серію копань за 400мс
...
// ОПТИМ: зберігаємо тільки якщо є зміни, кожні 30с
...
// ОПТИМ: pushPos кожні 500мс (було 100мс) + поріг 8px (було 4px)
// → в 5 разів менше записів позиції
...
// ОПТИМ: onSnapshot на колекцію — читає всіх гравців
// При 4 гравцях і pushPos 500мс: 4×2×4 = 32 читання/хв (прийнятно)
...
function gameLoop(ts) {
  ...
  // ОПТИМ: pushPos кожні 500мс замість 100мс → в 5 разів менше записів
  _posPushMs += rawDt;
...
function doMineAction(dt) {
  ...
  if (_mineProgress >= hardness) {
    ...
    if (res) {
      spawnDrop(tileX*T + T/2, tileY*T + T/2, res, 1);
