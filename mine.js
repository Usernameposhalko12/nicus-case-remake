...
function buildMineHTML() {
  ...
  if (!root) return;
  // Визначаємо тач-пристрій за наявністю touch подій, а не шириною екрану
  ...
  root.innerHTML = `
    <div id="mine-wrap">
      ...
      <!-- Кнопка режиму (копати / будувати) — по центру зверху -->
      <button id="mine-mode-btn" onclick="window.mineToggleMode()"></button>

      <!-- Тач-кнопки — завжди в DOM, показуємо через клас -->
      <div id="mine-touch-ctrl" class="${isTouch ? 'touch-visible' : 'touch-hidden'}">
        ...
        <!-- Стрибок + Копати/Будувати праворуч -->
      ...
      <!-- Хотбар — всередині wrap, над тач-кнопками -->
...
function resizeCanvas() {
  ...
  // Оновлюємо позицію хотбара залежно від наявності тач-кнопок
...
function injectMineStyles() {
  ...
  s.textContent = `
    ...
    #mine-wrap {
      position: relative;
      width: 100%;
      /* Висота = вікно мінус navbar(58) мінус bottom-nav(68) мінус page-header(~40) */
    ...
    /* ── Кнопка режиму — зверху по центру ── */
    #mine-mode-btn {
      position: absolute;
      top: 8px;
      left: 50%;
    ...
    /* ── Хотбар — всередині wrap, над кнопками ── */
    #mine-hotbar-wrap {
      position: absolute;
      /* На тач: вище тач-кнопок (96px висота кнопок + 4px відступ) */
      bottom: 104px;
      left: 50%;
      transform: translateX(-50%);
      z-index: 25;
    ...
    /* На не-тач пристроях (клас touch-hidden на touch-ctrl) — хотбар внизу */
    #mine-touch-ctrl.touch-hidden ~ #mine-hotbar-wrap,
    .touch-hidden ~ #mine-hotbar-wrap {
      bottom: 8px;
    }
    ...
    .mhb.mhb-sel {
      border-color: #f5b100;
      background: rgba(245,177,0,0.2);
      box-shadow: 0 0 10px rgba(245,177,0,0.45);
    }
    ...
    #mine-touch-ctrl {
      ...
      padding: 0 12px 8px;
      z-index: 30;
      /* Напівпрозорий фон щоб відокремити від гри */
      background: linear-gradient(to top, rgba(0,0,0,0.35), transparent);
    }

    /* Показуємо на тач-пристроях */
    ...
    .mtbtn:active {
      background: rgba(10,180,204,0.3);
      border-color: rgba(10,180,204,0.7);
      transform: scale(0.93);
    }

    .mtbtn-jump {
      background: rgba(10,180,204,0.2);
      border-color: rgba(10,180,204,0.45);
      font-size: 20px;
    }

    .mtbtn-act {
      background: rgba(240,125,40,0.22);
      border-color: rgba(240,125,40,0.5);
      font-size: 24px;
    }

    .mtbtn-act:active {
      background: rgba(240,125,40,0.45);
      border-color: rgba(240,125,40,0.85);
    }
