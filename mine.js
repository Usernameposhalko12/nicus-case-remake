// ============================================
// MINE.JS — НІКУС КЕЙС РЕМЕЙК v4
// Fixes: AFK heartbeat, slate block, jump fix,
//        NikisOre spawn, crafting table + items
// ============================================
...
// FIX #1: AFK — heartbeat кожні 8 секунд незалежно від руху
const HEARTBEAT_INTERVAL_MS = 8000;
// Гравець вважається активним якщо heartbeat < 20с тому
...
const T_WATER  = 14;
const T_SLATE  = 15;
const T_CRAFT  = 16; // FIX #5: верстак
...
const DROP_RESOURCE = {
  ...
  [T_CRAFT]:"wood", // верстак розбивається — дає дерево
};
...
// ══════════════════════════════════════════
// FIX #6: ПРЕДМЕТИ (характеристики)
// ══════════════════════════════════════════

const ITEM_DEFS = {
  iron_sword:    { type:"sword",   mat:"iron",    damage:1.0,  durability:100, desc:"Урон: 1.0D, міцність 100" },
  iron_armor:    { type:"armor",   mat:"iron",    reduce:1.0,  durability:100, desc:"Захист: 1R, міцність 100" },
  iron_pickaxe:  { type:"pickaxe", mat:"iron",    speed:1.0,   durability:100, desc:"Швидкість: 1.0x, міцність 100" },
  gold_sword:    { type:"sword",   mat:"gold",    damage:0.9,  vsMonster:1.3, durability:60, slowEffect:true, desc:"Уповільнює монстрів 20% на 2с" },
  gold_armor:    { type:"armor",   mat:"gold",    reduce:0.7,  durability:60,  slowAttacker:true, desc:"Атакуючий уповільнюється 20%" },
  gold_pickaxe:  { type:"pickaxe", mat:"gold",    speed:1.0,   dropBonus:0.05, durability:60, desc:"+5% дроп бонус" },
  diamond_sword: { type:"sword",   mat:"diamond", damage:2.0,  vsPlayer:1.25,  durability:200, desc:"Урон: 2D, vs гравець x1.25" },
  diamond_armor: { type:"armor",   mat:"diamond", reduce:0.67, fullSetBonus:1.5, durability:200, desc:"Повний сет: x1.5 захист" },
  diamond_pickaxe:{ type:"pickaxe",mat:"diamond", speed:1.5,   rareChance:0.05, durability:200, desc:"Швидк. 1.5x, +5% шанс руди" },
  nicus_sword:   { type:"sword",   mat:"nicus",   damage:3.0,  vsPlayer:1.5,   durability:400, desc:"Найсильніший меч" },
  nicus_armor:   { type:"armor",   mat:"nicus",   reduce:0.5,  durability:400, desc:"Найкращий захист" },
  nicus_pickaxe: { type:"pickaxe", mat:"nicus",   speed:2.0,   rareChance:0.1,  durability:400, desc:"Швидк. 2x, +10% шанс руди" },
};

// Рецепти крафту (верстак)
const CRAFT_RECIPES = [
  { result:"iron_sword",     count:1, needs:{ iron:3, stone:1 } },
  { result:"iron_armor",     count:1, needs:{ iron:5 } },
  { result:"iron_pickaxe",   count:1, needs:{ iron:3, wood:2 } },
  { result:"gold_sword",     count:1, needs:{ gold:3, stone:1 } },
  { result:"gold_armor",     count:1, needs:{ gold:5 } },
  { result:"gold_pickaxe",   count:1, needs:{ gold:3, wood:2 } },
  { result:"diamond_sword",  count:1, needs:{ diamond:3, stone:1 } },
  { result:"diamond_armor",  count:1, needs:{ diamond:5 } },
  { result:"diamond_pickaxe",count:1, needs:{ diamond:3, wood:2 } },
  { result:"nicus_sword",    count:1, needs:{ nicus_ore:3, diamond:1 } },
  { result:"nicus_armor",    count:1, needs:{ nicus_ore:5, diamond:1 } },
  { result:"nicus_pickaxe",  count:1, needs:{ nicus_ore:3, diamond:2 } },
];

// Рецепт верстака
const WORKBENCH_RECIPE = { wood: 4 };
...
// FIX #3: стрибок — прибираємо будь-який cooldown, додаємо coyoteTime
let _coyoteTimer = 0; // кількість тіків після того як злетіли з платформи
...
// FIX #5: верстак UI
let _craftOpen = false;

// Екіпірований предмет
let _equippedItem = null; // null або ключ з _inventory
...
async function showLobby(uid) {
  ...
  const [profSnap] = await Promise.all([
  ...
  const activePlayers = playersSnap.docs.filter(d => {
    const data = d.data();
    // FIX #1: використовуємо PLAYER_ACTIVE_MS замість 20000
    return d.id !== "lobby" && (now - (data.ts || 0)) < PLAYER_ACTIVE_MS;
  });
    ...
    <div id="mine-lobby">
      <div class="mlobby-card">
        ...
        <div class="mlobby-rules">
          <div>📋 Правила:</div>
          <div>• Максимум ${MAX_PLAYERS} гравці</div>
...
// FIX #1: heartbeat — пушимо позицію навіть якщо гравець стоїть
async function pushHeartbeat(uid) {
  ...
  try {
    await setDoc(doc(db, "minePlayers", uid), {
      ...
      dir: p.dir,
      mining: p.mining,
      ts: Date.now(), // завжди оновлюємо timestamp
    }, { merge: true });
    // Оновлюємо lastPushedPos щоб не дублювати pushPos
  ...
  root.innerHTML = `
    <div id="mine-wrap">
      ...
      <div id="mine-hotbar-wrap">
        ...
        <button id="mine-craft-btn" onclick="window.mineToggleCraft()" title="Верстак (4 дерева)">⋯</button>
      </div>
      <div id="mine-craft-panel" style="display:none;"></div>
      <div id="mine-log"></div>
    </div>
  `;
}
...
function stoneLayerTile(x, y, surf) {
  ...
  // FIX #2: сланець — правильно відображається
...
function coreLayerTile(x, y) {
  ...
  // FIX #4: NikisOre збільшений шанс (було 0.007 → 0.018) і спавниться глибше
...
function subscribeWorldMods() {
  _unsubWorld = onSnapshot(doc(db, "mineWorld", "main"), snap => {
    ...
    oldKeys.forEach(k => { if (!newKeys.has(k)) delete _worldMod[k]; });
...
function subscribeOthers(myUid) {
  _unsubOthers = onSnapshot(collection(db, "minePlayers"), snap => {
    ...
    snap.docs.forEach(d => {
      ...
      const data = d.data();
      // FIX #1: використовуємо PLAYER_ACTIVE_MS
      if (now - (data.ts || 0) > PLAYER_ACTIVE_MS) {
...
function update(dt) {
  ...
  // FIX #3: coyote time — можна стрибнути протягом 6 тіків після зльоту з платформи
  if (p.onGround) _coyoteTimer = 6;
  else if (_coyoteTimer > 0) _coyoteTimer--;

  // FIX #3: стрибок без cooldown — просто перевіряємо coyoteTimer
  ...
  // FIX #5: перевірка чи гравець поруч з верстаком
  checkNearWorkbench();
...
// FIX #3: resolveAxisX — виправлено вузькі проходи
  ...
  for (const cy of ys) {
    ...
    if (p.vx > 0) {
      if (isSolidTile(tx2, Math.floor(cy / T))) {
        p.x = tx2 * T - p.w; p.vx = 0; return;
      }
    } else {
      if (isSolidTile(tx1, Math.floor(cy / T))) {
        p.x = (tx1 + 1) * T; p.vx = 0; return;
      }
    }
  }
}

// FIX #3: resolveAxisY — перевіряємо isOnGround правильно
...
function doMineAction(dt) {
  ...
  // FIX #5: верстак — клік відкриває меню замість копання
  if (tile === T_CRAFT) {
    resetMine(p);
    openWorkbenchUI();
    return;
  ...
  // FIX #6: кирка прискорює копання
  let speedMult = 1.0;
  if (_equippedItem) {
    const def = ITEM_DEFS[_equippedItem];
...
function doBuild() {
  ...
  // FIX #5: якщо в хотбарі вибрано "workbench" — ставимо верстак
...
function checkNearWorkbench() {
  ...
  if (near !== _nearWorkbench) {
    _nearWorkbench = near;
    const btn = document.getElementById("mine-craft-btn");
    if (btn) btn.style.background = near
...
function openWorkbenchUI() {
  if (_craftOpen) { closeCraftPanel(); return; }
  _craftOpen = true;
  renderCraftPanel();
}

window.mineToggleCraft = function() {
  if (_craftOpen) closeCraftPanel();
  else { _craftOpen = true; renderCraftPanel(); }
};

function closeCraftPanel() {
  _craftOpen = false;
  const el = document.getElementById("mine-craft-panel");
...
function renderCraftPanel() {
  ...
  // Перевіряємо верстак поряд
  const needWorkbench = true;
  const hasWorkbench  = _nearWorkbench;

  let html = `
    <div class="mcraf-header">
      <span>🪵 Верстак</span>
      <button onclick="window.mineCloseCraft()" style="background:none;border:none;color:#fff;font-size:18px;cursor:pointer;">✕</button>
    </div>
    ${!hasWorkbench ? `<div class="mcraf-warn">⚠️ Підійди ближче до верстаку (або постав 4 дерева → E→ клік)</div>` : ""}
    <div class="mcraf-place-row">
      <b style="font-size:11px;color:#aaa;">🪵 Поставити верстак:</b>
      <span style="font-size:11px;color:#f5b100;">Потрібно: 4 🪵 (є: ${_inventory["wood"]||0})</span>
      <button class="mcraf-btn" onclick="window.minePlaceWorkbench()" ${(_inventory["wood"]||0)>=4?'':'disabled'}>📦 Поставити</button>
    </div>
  ...
  CRAFT_RECIPES.forEach((rec, i) => {
    ...
    if (!def) return;
    const canCraft = hasWorkbench && Object.entries(rec.needs).every(([k,v]) => (_inventory[k]||0) >= v);
    ...
    const icon = generateItemIconDataUrl(rec.result);
    html += `
      <div class="mcraf-item">
        <img src="${icon}" width="32" height="32" style="image-rendering:pixelated;border-radius:4px;">
        <div class="mcraf-item-info">
          <div class="mcraf-item-name">${RES_DATA[rec.result]?.name||rec.result}</div>
          <div class="mcraf-item-desc">${def.desc||""}</div>
          <div class="mcraf-item-needs">${needsHtml}</div>
        </div>
        <button class="mcraf-btn ${canCraft?'':'disabled'}" onclick="window.mineCraft(${i})" ${canCraft?'':'disabled'}>Крафт</button>
      </div>
    `;
  });
  html += `</div>`;
...
window.mineCloseCraft = closeCraftPanel;

window.minePlaceWorkbench = function() {
  // Ставимо верстак перед гравцем
  if (!_player) return;
...
window.mineCraft = function(recipeIdx) {
  const rec = CRAFT_RECIPES[recipeIdx];
  if (!rec) return;
...
window.mineEquipItem = function(itemKey) {
  if (_equippedItem === itemKey) {
    _equippedItem = null;
    addLog("🎒 Знято: " + (RES_DATA[itemKey]?.name||itemKey));
  } else {
    _equippedItem = itemKey;
    addLog("⚔️ Екіпіровано: " + (RES_DATA[itemKey]?.name||itemKey));
  }
  updateEquipHUD();
};
...
function updateEquipHUD() {
  ...
  if (!el) return;
  // Показуємо всі скрафтовані предмети
  ...
  el.innerHTML = craftedItems.map(k => {
    ...
    const icon = generateItemIconDataUrl(k);
    const isEq = _equippedItem === k;
    return `<div class="mequip-item${isEq?' mequip-active':''}" onclick="window.mineEquipItem('${k}')" title="${RES_DATA[k]?.name||k}: ${def?.desc||''}">
      <img src="${icon}" width="20" height="20" style="image-rendering:pixelated;">
      <span>${_inventory[k]}</span>
    </div>`;
  }).join("");
}

// ══════════════════════════════════════════
// FIX #6: CANVAS-ГЕНЕРАЦІЯ ІКОНОК ПРЕДМЕТІВ
// ══════════════════════════════════════════

const _iconCache = {};

function generateItemIconDataUrl(itemKey) {
  if (_iconCache[itemKey]) return _iconCache[itemKey];
  const c = document.createElement("canvas");
  c.width = 32; c.height = 32;
  const ctx = c.getContext("2d");
  drawItemIcon(ctx, itemKey, 32);
  const url = c.toDataURL();
  _iconCache[itemKey] = url;
  return url;
}

function drawItemIcon(ctx, itemKey, size) {
  const def = ITEM_DEFS[itemKey];
  if (!def) return;

  const MAT_COLORS = {
    iron:    ["#b8b8cc", "#8888a0", "#d0d0e0"],
    gold:    ["#f5c000", "#c89000", "#ffd840"],
    diamond: ["#40e0ff", "#00aacc", "#80f0ff"],
    nicus:   ["#cc80ff", "#8833cc", "#e0b0ff"],
  };
  const cols = MAT_COLORS[def.mat] || ["#888","#555","#aaa"];
  const [mid, dark, light] = cols;
  ...
  if (def.type === "sword") {
    ...
    ctx.fillRect(size*0.4, size*0.65, size*0.2, size*0.3);
    // Гарда
    ctx.fillStyle = dark;
    ctx.fillRect(size*0.25, size*0.6, size*0.5, size*0.08);
    // Клинок
    ctx.fillStyle = mid;
    ctx.fillRect(size*0.42, size*0.1, size*0.16, size*0.52);
    // Блиск
    ctx.fillStyle = light;
    ctx.fillRect(size*0.44, size*0.12, size*0.05, size*0.4);

  } else if (def.type === "armor") {
    // Нагрудник
    ctx.fillStyle = mid;
    ctx.fillRect(size*0.2, size*0.25, size*0.6, size*0.55);
    // Плечі
    ctx.fillRect(size*0.12, size*0.2, size*0.18, size*0.2);
    ctx.fillRect(size*0.7, size*0.2, size*0.18, size*0.2);
    // Блиск
    ctx.fillStyle = light;
    ctx.fillRect(size*0.28, size*0.3, size*0.1, size*0.35);
    // Тінь
    ctx.fillStyle = dark;
    ctx.fillRect(size*0.6, size*0.3, size*0.1, size*0.4);
    // Обводка
    ctx.strokeStyle = dark;
    ctx.lineWidth = 1;
    ctx.strokeRect(size*0.2, size*0.25, size*0.6, size*0.55);
    ...
    ctx.fillRect(-size*0.05, -size*0.45, size*0.1, size*0.7);
    ctx.restore();
    // Голова кирки
    ctx.fillStyle = mid;
    ctx.save();
    ...
    ctx.rotate(Math.PI/4);
    ctx.fillRect(-size*0.22, -size*0.07, size*0.44, size*0.14);
    // Гострий кінець
    ctx.fillRect(-size*0.22, -size*0.07, size*0.1, size*0.22);
    ctx.restore();
    ctx.fillStyle = light;
    ctx.save();
    ctx.translate(size*0.18, size*0.18);
    ctx.rotate(Math.PI/4);
    ctx.fillRect(-size*0.2, -size*0.05, size*0.15, size*0.04);
    ctx.restore();
  }
}
...
function spawnDropSafe(tileX, tileY, res, amt) {
  ...
  _drops.push({ x:wx, y:wy, vx:(Math.random()-0.5)*1.5, vy:0, res, amt, age:0, bob:Math.random()*Math.PI*2 });
}
...
function drawTiles(ctx, W, H) {
  ...
  for (let ty2 = sy; ty2 <= ey; ty2++) {
    for (let tx2 = sx; tx2 <= ex; tx2++) {
      ...
      // FIX #5: верстак — малюємо спрощену іконку
      if (tile === T_CRAFT) {
        ctx.fillStyle = "#a0622a";
        ctx.fillRect(px, py2, T, T);
        ctx.fillStyle = "#c87a30";
        ctx.fillRect(px+2, py2+2, T-4, T-4);
        // Решітка
        ctx.fillStyle = "#8b5e3c";
        ctx.fillRect(px+T/2-1, py2+2, 2, T-4);
        ctx.fillRect(px+2, py2+T/2-1, T-4, 2);
        ctx.fillStyle = "rgba(255,200,100,0.5)";
        ctx.fillRect(px+2, py2+2, 5, 5);
        ctx.globalAlpha = 1;
        if (vis < 1) { ctx.fillStyle=`rgba(0,0,0,${1-vis})`; ctx.fillRect(px,py2,T,T); }
        continue;
      }

      // FIX #2: сланець — правильний рендер з блиском
      ctx.fillStyle = color;
      ctx.fillRect(px, py2, T, T);
      ...
      // Блиск для руди (включно зі сланцем та нікус-рудою)
      if ([T_IRON,T_LEAD,T_GOLD,T_URAN,T_DIAM,T_NICUS,T_SLATE].includes(tile)) {
        ctx.fillStyle = "rgba(255,255,255,0.28)";
        ctx.fillRect(px+3, py2+3, 4, 4);
        ctx.fillRect(px+T-7, py2+T-7, 3, 3);
        // FIX #4: NikisOre — додатковий фіолетовий блиск
        if (tile === T_NICUS) {
          ctx.fillStyle = "rgba(200,100,255,0.4)";
          ctx.fillRect(px+2, py2+2, 6, 6);
          ctx.fillRect(px+T-8, py2+T-8, 5, 5);
        }
      }
...
function drawCharacter(ctx, x, y, w, h, name, hpFrac, dir, isMining, color, isSelf, isMoving) {
  ...
  ctx.beginPath();
  ctx.moveTo(cx, bodyBot); ctx.lineTo(cx-4, bodyBot+8+legSwing); ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(cx, bodyBot); ctx.lineTo(cx+4, bodyBot+8-legSwing); ctx.stroke();
  ...
  if (isMining) {
    ...
    ctx.beginPath();
    ctx.moveTo(cx, armY); ctx.lineTo(cx+dir*12, armY+sw); ctx.stroke();
    ctx.strokeStyle = "#f5b100";
    ctx.lineWidth = 3;
    const pickX = cx+dir*12, pickY = armY+sw;
    ctx.beginPath();
    ctx.moveTo(pickX, pickY); ctx.lineTo(pickX+dir*8, pickY+7); ctx.stroke();
    ctx.strokeStyle = color; ctx.lineWidth = 2.5;
  } else {
    const armSwing = isMoving ? Math.sin(walkPhase)*5 : 0;
    ctx.beginPath(); ctx.moveTo(cx, armY); ctx.lineTo(cx-7, armY+8+armSwing); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx, armY); ctx.lineTo(cx+7, armY+8-armSwing); ctx.stroke();
  }
...
function drawDrops(ctx) {
  ...
  _drops.forEach(d => {
    ...
    ctx.fill();
    // FIX #6: крафтовані предмети — малюємо іконку
    if (ITEM_DEFS[d.res]) {
      const url = generateItemIconDataUrl(d.res);
      const img = new Image();
      img.src = url;
      ctx.drawImage(img, d.x-10, d.y+bob-10, 20, 20);
    } else {
...
function updateResHUD() {
  ...
  if (!el) return;
  const entries = Object.entries(_inventory)
    .filter(([k,v]) => v > 0 && !ITEM_DEFS[k])
    .slice(0, 6);
...
function updateHotbarUI() {
  ...
  if (!el) return;
  // Хотбар: звичайні ресурси + "workbench" якщо є дерево
  const slots = [..._hotbar];
  el.innerHTML = slots.map((res, i) => {
...
const _onKeyDown = (e) => {
  ...
  if (e.key === "f" || e.key === "F") { window.mineToggleCraft(); }
...
const _onCanvasTouchStart = (e) => {
  for (const t of e.changedTouches) {
    ...
    if (el && el.closest("#mine-craft-panel")) continue;
    const r = _canvas.getBoundingClientRect();
    _mouseWorld.x = (t.clientX - r.left) * (_canvas.width/r.width)  + _cam.x;
    _mouseWorld.y = (t.clientY - r.top)  * (_canvas.height/r.height) + _cam.y;
    _mineActive = true;
...
function injectMineStyles() {
  ...
  s.textContent = `
    ...
    .mequip-item { display:flex;align-items:center;gap:3px;background:rgba(0,0,0,0.65);border:1.5px solid rgba(255,255,255,0.15);border-radius:8px;padding:3px 7px;cursor:pointer;transition:all 0.15s; }
    .mequip-item.mequip-active { border-color:#f5b100;background:rgba(245,177,0,0.2);box-shadow:0 0 8px rgba(245,177,0,0.4); }
    .mequip-item span { font-size:10px;color:#f5b100;font-weight:700; }
    ...
    #mine-craft-btn { width:40px;height:46px;background:rgba(0,0,0,0.55);border:2px solid rgba(255,180,80,0.4);border-radius:8px;color:#f5b100;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all 0.2s;-webkit-tap-highlight-color:transparent;touch-action:manipulation; }
    #mine-craft-btn:active { transform:scale(0.9); }
    ...
    /* ── Верстак панель ── */
    #mine-craft-panel { position:absolute;bottom:160px;left:50%;transform:translateX(-50%);z-index:30;background:rgba(12,20,30,0.97);border:1.5px solid rgba(200,120,48,0.5);border-radius:14px;width:min(340px,95vw);max-height:65vh;overflow-y:auto;box-shadow:0 8px 32px rgba(0,0,0,0.7);scrollbar-width:thin;scrollbar-color:rgba(200,120,48,0.3) transparent; }
    .mcraf-header { display:flex;justify-content:space-between;align-items:center;padding:12px 14px 8px;border-bottom:1px solid rgba(200,120,48,0.3);font-weight:800;color:#f5c060;font-size:15px; }
    ...
    .mcraf-place-row { display:flex;flex-wrap:wrap;align-items:center;gap:6px;padding:8px 12px;border-bottom:1px solid rgba(200,120,48,0.2);font-size:12px; }
    ...
    .mcraf-btn:hover { filter:brightness(1.1); }
