...
const SKY_H    = 40;          // y тайлу де починається поверхня
const DEEP_TOP = 160;
const CORE_TOP = 220;
...
// Кольори тайлів (для рендеру без PNG)
const TILE_COLOR = {
  [T_GRASS]:  "#4a8a2a",
  [T_DIRT]:   "#8b5e3c",
  [T_STONE]:  "#7a7a8a",
...
// Що можна розмістити (будівництво)
const PLACEABLE = {
  stone: T_STONE, dirt: T_DIRT, wood: T_WOOD, sand: T_SAND,
};

// Твердість тайлів (мс для копання при базовій кирці)
const TILE_HARDNESS = {
  [T_GRASS]:  300,
  [T_DIRT]:   300,
  [T_SAND]:   200,
...
let _world       = null;      // Uint8Array
let _worldMod    = {};        // { "tx,ty": tileId }
let _player      = null;
let _others      = {};        // uid → { x,y,name,dir,mining,ts }
let _canvas      = null;
let _ctx         = null;
let _cam         = { x:0, y:0 };
...
// Копання
let _mineProgress = 0;        // мс
let _mineTile     = null;     // { tx, ty } поточний тайл
let _mineActive   = false;    // чи тримаємо ЛКМ/тач
...
let _touchMineId = null;   // ідентифікатор тач-точки для копання
...
export async function initMinePage() {
  ...
  // Безпечний спавн — гарантовано не в блоці
  ...
  if (profSnap.exists()) {
    ...
    // Перевіряємо чи збережена позиція безпечна
...
function isSpawnSafe(tx, ty) {
  // Перевіряємо що гравець (2 тайли висотою) не перетинає тверді блоки
      ...
      <div id="mine-touch-ctrl">
        ...
        <div id="mine-touch-left">
          <button class="mtbtn"
            ...
            ontouchcancel="event.preventDefault();window.mineTouch('left',0)">◀</button>
          <button class="mtbtn"
            ontouchstart="event.preventDefault();window.mineTouch('right',1)"
            ontouchend="event.preventDefault();window.mineTouch('right',0)"
            ontouchcancel="event.preventDefault();window.mineTouch('right',0)">▶</button>
        </div>
        <!-- Стрибок і копання праворуч -->
        <div id="mine-touch-right">
          <button class="mtbtn mtbtn-jump"
            ...
            ontouchcancel="event.preventDefault();window.mineTouch('jump',0)">▲</button>
          <button class="mtbtn mtbtn-act" id="mine-touch-act"
            ontouchstart="event.preventDefault();window.mineTouchMine(1)"
            ontouchend="event.preventDefault();window.mineTouchMine(0)"
            ontouchcancel="event.preventDefault();window.mineTouchMine(0)">⛏</button>
        </div>
      </div>

      <!-- Інструкція (перша сесія) -->
      <div id="mine-hint" onclick="this.style.display='none'">
        📱 Тач по тайлу — копати<br>⛏ кнопка — копати вперед<br>E / ПКМ — будувати
      </div>
...
function resizeCanvas() {
  ...
  _canvas.height = wrap.clientHeight || Math.max(320, window.innerHeight - 120);
}
...
function generateWorld() {
  ...
  for (let x = 0; x < W_TILES; x++) {
    ...
    // Дерева
    if (Math.random() < 0.06 && x > 5 && x < W_TILES - 5) {
      const surf2 = hmap[x];
...
function carveCaves() {
  ...
  for (let w = 0; w < count; w++) {
    ...
    let cy  = (SKY_H + 10) + Math.floor(Math.random() * (H_TILES - SKY_H - 15));
...
function gameLoop(ts) {
  ...
  _lastTs = ts;
  // Обмежуємо dt щоб не було великого стрибка при втраті фокусу
...
// ── КОЛІЗІЯ ПО X (ліворуч/праворуч) ───────────────────────
function resolveAxisX(p) {
  if (p.vx === 0) return;

  // Перебираємо кути AABB
  const corners = [
    { cx: p.x,        cy: p.y },
    { cx: p.x,        cy: p.y + p.h - 1 },
    { cx: p.x + p.w - 1, cy: p.y },
  ...
  for (const c of corners) {
    ...
    if (p.vx > 0) {
      // Рухаємось вправо — відштовхуємось від лівого краю тайлу
      p.x = tx * T - p.w;
    } else {
      // Рухаємось вліво — відштовхуємось від правого краю тайлу
      p.x = (tx + 1) * T;
    }
    p.vx = 0;
    break;
...
// ── КОЛІЗІЯ ПО Y (верх/низ) ────────────────────────────────
function resolveAxisY(p) {
  // Перебираємо 4 точки
  const corners = [
    { cx: p.x + 2,        cy: p.y },           // верхній лівий
    { cx: p.x + p.w - 3,  cy: p.y },           // верхній правий
    { cx: p.x + 2,        cy: p.y + p.h - 1 }, // нижній лівий
    { cx: p.x + p.w - 3,  cy: p.y + p.h - 1 }, // нижній правий
  ];
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (p.vy > 0) {
      // Падаємо вниз — ставимо на поверхню тайлу
      p.y = ty * T - p.h;
      p.onGround = true;
    } else {
      // Летимо вгору — відштовхуємось від дна тайлу
      p.y = (ty + 1) * T;
    }
    p.vy = 0;
    break;
...
function doMineAction(dt) {
  ...
  if (tile === T_AIR || tile === T_WATER || tile === T_BEDROCK) {
    resetMine(p); return;
  }

  // Новий тайл — скидаємо прогрес
...
function doBuild() {
  ...
  updateHotbarUI();
  // Throttle: не будуємо кожен фрейм
  _mineActive = false;
  _keys["Mouse0"] = false;
}
...
function render() {
  ...
  // Небо — градієнт залежно від глибини
...
// Тач по канвасу — переміщення прицілу
const _onTouchMove = (e) => {
  if (!_canvas || !_player) return;
  // Оновлюємо позицію прицілу лише від не-кнопкових дотиків
  for (const t of e.changedTouches) {
    // Якщо touch-mine активний — не рухаємо приціл від цього пальця
    if (t.identifier === _touchMineId) continue;
    const el = document.elementFromPoint(t.clientX, t.clientY);
    // Якщо палець на кнопці — ігноруємо
...
// Тап по канвасу — встановлюємо ціль для копання
...
// ══════════════════════════════════════════
// ГЛОБАЛЬНІ ФУНКЦІЇ (викликаються з HTML)
// ══════════════════════════════════════════

window.mineTouch = (btn, v) => {
  _touch[btn] = !!v;
};
...
window.mineTouchMine = (v) => {
  ...
  // Якщо копаємо кнопкою — ціль прямо перед гравцем
  if (v && _player) {
    const p = _player;
    const targetX = p.x + p.dir * (p.w + T * 1.5);
    const targetY = p.y + p.h * 0.5;
    _mouseWorld.x = targetX;
    _mouseWorld.y = targetY;
  }
};
...
window.mineToggleMode = () => {
  ...
  updateModeBtn();
  // Якщо переходимо у режим будівництва — виводимо підказку
...
function injectMineStyles() {
  ...
  s.textContent = `
    ...
    #page-mine .page-header { padding: 8px 12px; margin-bottom: 0; border-bottom: none; }
    ...
    .mbar-hp {
      ...
      background: linear-gradient(90deg, #5ecb3e, #8bff60);
    }
    ...
    .mhb.mhb-sel {
      ...
      box-shadow: 0 0 10px rgba(245,177,0,0.4), inset 0 0 6px rgba(245,177,0,0.1);
    }

    .mhb-ico { font-size: 18px; line-height: 1; }
    .mhb-cnt {
      position: absolute;
    ...
    #mine-mode-btn:active { transform: translateX(-50%) scale(0.95); }
    ...
    @media (min-width: 700px) {
      #mine-touch-ctrl { display: none !important; }
    }
    ...
    .mtbtn:active {
      background: rgba(10,180,204,0.25);
      border-color: rgba(10,180,204,0.6);
    }

    .mtbtn-jump {
      background: rgba(10,180,204,0.18);
      border-color: rgba(10,180,204,0.4);
    }

    .mtbtn-act {
      background: rgba(240,125,40,0.2);
      border-color: rgba(240,125,40,0.45);
      font-size: 22px;
    }

    .mtbtn-act:active {
      background: rgba(240,125,40,0.4);
      border-color: rgba(240,125,40,0.8);
    }
