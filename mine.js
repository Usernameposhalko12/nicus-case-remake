...
function update(dt) {
  ...
  // ── ІНТЕРПОЛЯЦІЯ ІНШИХ ГРАВЦІВ ──
  // Кожен кадр плавно рухаємо поточну позицію до цільової
  Object.values(_others).forEach(o => {
    ...
    if (dist < 1) {
      ...
      // Рухаємось до цілі зі швидкістю пропорційною до відстані
      // Це забезпечує плавний рух навіть при великій затримці
      const speed = Math.min(dist, INTERP_SPEED * dt * 3);
      o.x += (dx / dist) * speed;
      o.y += (dy / dist) * speed;
    }
  });
